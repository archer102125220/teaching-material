...
try {
  const credentials = await import('../google-key/isu-member-90f333a85fc9.mjs');
  extractSheets(
    {
  ...
  const { default: enJsonData } = await import('./locales/en.json', {
    assert: { type: 'json' }
  });
  const { default: zhTwJsonData } = await import('./locales/zh-tw.json', {
    assert: { type: 'json' }
  });
  handleJson(null, undefined, { en: enJsonData, zhTw: zhTwJsonData });
}

function handleJson(err, data = {}, defaultData) {
  if (err) throw err;
  let en = {
    system: {
  ...
  if (Object.keys(data || {}).length <= 0) {
    if (typeof defaultData?.en === 'object' && defaultData.en !== null) {
      const defaultEn = defaultData.en;
      // const enKeys = Object.keys(en);
      // const defaultEnKeys = Object.keys(defaultEn);
      // const keyList = Array.from(new Set([...enKeys, ...defaultEnKeys]));
      // keyList.forEach((key) => {
      //   if (en[key] === undefined || en[key] === null) {
      //     en[key] = defaultEn[key];
      //   } else if (
      //     typeof en[key] === 'object' &&
      //     typeof defaultEn[key] === 'object'
      //   ) {
      //     en[key] = { ...defaultEn[key], ...en[key] };
      //   } else {
      //     console.log(en[key], key);
      //   }
      // });
      en = handleDefaultJson(
        JSON.parse(JSON.stringify(en)),
        JSON.parse(JSON.stringify(defaultEn))
      );
    }

    if (typeof defaultData?.zhTw === 'object' && defaultData.zhTw !== null) {
      const defaultZhTw = defaultData.zhTw
      // const zhTwKeys = Object.keys(zhTw);
      // const defaultZhTwKeys = Object.keys(defaultZhTw);
      // const keyList = Array.from(new Set([...zhTwKeys, ...defaultZhTwKeys]));
      zhTw = handleDefaultJson(
        JSON.parse(JSON.stringify(zhTw)),
        JSON.parse(JSON.stringify(defaultZhTw))
      );
    }
  } else {
    sheetsToExtract.forEach(function (_sheetsToExtract) {
      ...
      sheets.forEach(function (_sheets) {
        if (_sheets.key !== null && _sheets.key !== undefined) {
...
function handleDefaultJson(data, defaultData) {
  const dataKeys = Object.keys(data);
  const defaultDataKeys = Object.keys(defaultData);
  const keyList = Array.from(new Set([...dataKeys, ...defaultDataKeys]));
  keyList.forEach((key) => {
    if (data[key] === undefined || data[key] === null) {
      ...
      data[key] = { ...defaultData[key], ...data[key] };
    } else {
