...
class DampedHarmonic {
  ...
  public constructor(a: number, b: number, c: number, initialValue: number, initialDerivative: number) {
    window.assert && window.assert(isFinite(a) && a !== 0);
    window.assert && window.assert(isFinite(b));
    window.assert && window.assert(isFinite(c) && c !== 0);
    window.assert && window.assert(isFinite(initialValue));
    window.assert && window.assert(isFinite(initialDerivative));
    ...
    window.assert && window.assert(this.dampingConstant >= 0, 'a and b should share the same sign');
    window.assert && window.assert(this.angularFrequencySquared > 0, 'a and c should share the same sign');
  ...
  public getValue(t: number): number {
    if (this.solutionType === SolutionType.CRITICALLY_DAMPED) {
      window.assert && window.assert(this.angularFrequency !== undefined);

      return (this.c1 + this.c2 * t) * Math.exp(-this.angularFrequency! * t);
    }
    else if (this.solutionType === SolutionType.UNDER_DAMPED) {
      window.assert && window.assert(this.frequency !== undefined);
      ...
      window.assert && window.assert(this.positiveRoot !== undefined);
      window.assert && window.assert(this.negativeRoot !== undefined);
