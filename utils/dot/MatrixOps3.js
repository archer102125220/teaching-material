...
const MatrixOps3 = {
   ...
   */
  index3(row, col) {
    window.assert && window.assert(row >= 0 && row < 3);
    window.assert && window.assert(col >= 0 && col < 3);
    return 3 * row + col;
  },
   ...
   */
  set3(matrix, result) {
    window.assert && window.assert(matrix.length >= 9);
    window.assert && window.assert(result.length >= 9);
    result[0] = matrix[0];
    result[1] = matrix[1];
    result[2] = matrix[2];
    result[3] = matrix[3];
  ...
  mult3(left, right, result) {
    window.assert && window.assert(left.length >= 9);
    window.assert && window.assert(right.length >= 9);
    window.assert && window.assert(result.length >= 9);
  ...
  setGivens3(result, cos, sin, idx0, idx1) {
    window.assert && window.assert(idx0 < idx1);
  ...
  qrAnnihilate3(q, r, row, col) {
    window.assert && window.assert(row > col); // only in the lower-triangular area
  ...
  setVectors3(columnVectors, result) {
    ...
    window.assert &&
      window.assert(result.length >= m * n, 'Array length check');
  ...
  getColumnVector3(m, n, matrix, columnIndex, result) {
    window.assert && window.assert(m === 3 && columnIndex < n);
  ...
  transpose(m, n, matrix, result) {
    window.assert && window.assert(matrix.length >= m * n);
    window.assert && window.assert(result.length >= n * m);
    window.assert &&
      window.assert(
        matrix !== result,
        'In-place modification not implemented yet'
      );
  ...
  mult(m, n, p, left, right, result) {
    window.assert && window.assert(left.length >= m * n);
    window.assert && window.assert(right.length >= n * p);
    window.assert && window.assert(result.length >= m * p);
    window.assert &&
      window.assert(
  ...
  permuteColumns(m, n, matrix, permutation, result) {
    ...
    window.assert && window.assert(matrix.length >= m * n);
    window.assert && window.assert(result.length >= m * n);

    for (let col = 0; col < n; col++) {
      const permutedColumnIndex = permutation.indices[col];
      for (let row = 0; row < m; row++) {
        result[this.index(m, n, row, col)] =
          matrix[this.index(m, n, row, permutedColumnIndex)];
      }
    }
  }
};
