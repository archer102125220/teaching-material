...
class Complex {
  ...
  equalsEpsilon(other, epsilon) {
    ...
    return (
      Math.max(
        Math.abs(this.real - other.real),
        Math.abs(this.imaginary - other.imaginary)
      ) <= epsilon
    );
  }
  ...
  static solveCubicRoots(a, b, c, d) {
    ...
    const D1_1 = b3
      ...
      .add(a2.times(d).multiply(Complex.real(27)));
    ...
    const discriminant1 = abc
      .times(d)
      .multiply(Complex.real(18))
      .add(b2.times(c2));
    const discriminant2 = b3
      .times(d)
      .multiply(Complex.real(4))
    ...
    if (discriminant1.equals(discriminant2)) {
      const simpleRoot = abc
        .times(Complex.real(4))
        .subtract(b3.plus(a2.times(d).multiply(Complex.real(9))))
        .divide(a.times(Delta0));
      const doubleRoot = a
        .times(d)
        .multiply(Complex.real(9))
        .subtract(b.times(c))
        .divide(Delta0.times(Complex.real(2)));
    ...
    if (D0_1.equals(D0_2)) {
      ...
      Ccubed = Delta1.plus(
        Delta1.times(Delta1)
          .subtract(
            Delta0.times(Delta0).multiply(Delta0).multiply(Complex.real(4))
          )
          .sqrt()
      ).divide(Complex.real(2));
    }
