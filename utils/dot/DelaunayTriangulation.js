...
class DelaunayTriangulation {
  ...
  constructor(points, constraints, options) {
    ...
    this.vertices = points.map((point, index) => {
      window.assert &&
        window.assert(point instanceof Vector2 && point.isFinite());
      ...
      window.assert &&
        window.assert(
          typeof secondIndex === 'number' &&
            isFinite(secondIndex) &&
            secondIndex % 1 === 0 &&
            secondIndex >= 0 &&
            secondIndex < points.length
        );
      window.assert && window.assert(firstIndex !== secondIndex);

      this.vertices[firstIndex].constrainedVertices.push(
        this.vertices[secondIndex]
      );
    }
    ...
    this.artificialMinVertex = new Vertex(
      new Vector2(
        bounds.minX - bounds.width * alpha,
        bounds.minY - bounds.height * alpha
      ),
      -1
    );
    // @private {Vertex} - Fake index -2
    this.artificialMaxVertex = new Vertex(
      new Vector2(
        bounds.maxX + bounds.width * alpha,
        bounds.minY - bounds.height * alpha
      ),
      -2
    );
    ...
    this.edges.push(
      new Edge(this.artificialMinVertex, this.artificialMaxVertex)
    );
    ...
    this.triangles.push(
      new Triangle(
        this.artificialMinVertex,
        this.artificialMaxVertex,
        this.bottomVertex,
        this.edges[1],
        this.edges[2],
        this.edges[0]
  ...
  step() {
    ...
    while (frontEdge) {
      ...
      if (x > frontEdge.endVertex.point.x) {
        ...
        window.assert && window.assert(leftOldEdge !== null);
  ...
  fillBorderTriangle(
    ...
    window.assert && window.assert(firstEdge instanceof Edge);
    window.assert && window.assert(secondEdge instanceof Edge);
    window.assert && window.assert(firstSideVertex instanceof Vertex);
    window.assert && window.assert(middleVertex instanceof Vertex);
    window.assert && window.assert(secondSideVertex instanceof Vertex);

    window.assert &&
      window.assert(
        middleVertex === firstEdge.startVertex ||
          middleVertex === firstEdge.endVertex,
        'middleVertex should be in firstEdge'
      );
    window.assert &&
      window.assert(
        middleVertex === secondEdge.startVertex ||
          middleVertex === secondEdge.endVertex,
        'middleVertex should be in secondEdge'
      );
    window.assert &&
      window.assert(
        firstSideVertex === firstEdge.startVertex ||
          firstSideVertex === firstEdge.endVertex,
        'firstSideVertex should be in firstEdge'
      );
    window.assert &&
      window.assert(
        secondSideVertex === secondEdge.startVertex ||
          secondSideVertex === secondEdge.endVertex,
  ...
  addHalfPiHeuristic(rightFrontEdge, leftFrontEdge) {
    window.assert &&
      window.assert(rightFrontEdge.endVertex === leftFrontEdge.startVertex);

    const middleVertex = rightFrontEdge.endVertex;

    while (
      rightFrontEdge.previousEdge &&
      Utils.triangleAreaSigned(
        middleVertex.point,
        rightFrontEdge.startVertex.point,
        rightFrontEdge.previousEdge.startVertex.point
      ) > 0 &&
      middleVertex.point
        .minus(rightFrontEdge.startVertex.point)
        .angleBetween(
          rightFrontEdge.previousEdge.startVertex.point.minus(
            rightFrontEdge.startVertex.point
          )
        ) <
        Math.PI / 2
      ...
      this.triangles.push(
        new Triangle(
          middleVertex,
          rightFrontEdge.startVertex,
          previousEdge.startVertex,
          previousEdge,
          newRightEdge,
          rightFrontEdge
        )
    ...
    while (
      leftFrontEdge.nextEdge &&
      Utils.triangleAreaSigned(
        middleVertex.point,
        leftFrontEdge.nextEdge.endVertex.point,
        leftFrontEdge.endVertex.point
      ) > 0 &&
      middleVertex.point
        .minus(leftFrontEdge.endVertex.point)
        .angleBetween(
          leftFrontEdge.nextEdge.endVertex.point.minus(
            leftFrontEdge.endVertex.point
          )
        ) <
        Math.PI / 2
      ...
      this.triangles.push(
        new Triangle(
          middleVertex,
          leftFrontEdge.nextEdge.endVertex,
          leftFrontEdge.endVertex,
          nextEdge,
          leftFrontEdge,
          newLeftEdge
        )
  ...
  constrainEdges(vertex, rightFrontEdge, leftFrontEdge) {
    window.assert && window.assert(vertex instanceof Vertex);
    window.assert && window.assert(rightFrontEdge instanceof Edge);
    window.assert && window.assert(leftFrontEdge instanceof Edge);
    window.assert && window.assert(vertex === rightFrontEdge.endVertex);
    window.assert && window.assert(vertex === leftFrontEdge.startVertex);
    ...
    for (let i = 0; i < vertex.constrainedVertices.length; i++) {
      ...
      if (
        bottomVertex === rightFrontEdge.startVertex ||
        bottomVertex === leftFrontEdge.endVertex
      ) {
        break;
      }
      ...
      let outsideRight =
        DelaunayTriangulation.vertexProduct(
          vertex,
          rightFrontEdge.startVertex,
          bottomVertex
        ) > 0;
      ...
      if (!outsideRight && !outsideLeft) {
        window.assert && window.assert(rightFrontEdge.triangles.length === 1);
        window.assert && window.assert(leftFrontEdge.triangles.length === 1);
        ...
        while (
          DelaunayTriangulation.vertexProduct(
            vertex,
            (nextVertex = currentTriangle
              .getEdgeOppositeFromVertex(vertex)
              .getOtherVertex(lastVertex)),
            bottomVertex
          ) < 0
        ) {
          currentTriangle = currentTriangle
            .getEdgeOppositeFromVertex(lastVertex)
            .getOtherTriangle(currentTriangle);
        ...
        rightEdges.push(
          currentTriangle.getEdgeOppositeFromVertex(
            currentEdge.getOtherVertex(lastVertex)
          )
        );
        window.assert &&
          window.assert(
            leftEdges[0].getOtherVertex(vertex).point.x <
              rightEdges[0].getOtherVertex(vertex).point.x
          );
      }
        ...
        if (outsideRight) {
          ...
          if (currentEdge.triangles.length > 1) {
            ...
            if (nextTriangle.hasVertex(bottomVertex)) {
              ...
              let nextEdge;
              if (
                nextTriangle.aEdge !== currentEdge &&
                nextTriangle.aEdge.intersectsConstrainedEdge(
                  vertex,
                  bottomVertex
                )
              ) {
                ...
                nextTriangle.bEdge !== currentEdge &&
                nextTriangle.bEdge.intersectsConstrainedEdge(
                  vertex,
                  bottomVertex
                )
              ) {
                ...
                nextTriangle.cEdge !== currentEdge &&
                nextTriangle.cEdge.intersectsConstrainedEdge(
                  vertex,
                  bottomVertex
                )
              ) {
              ...
              window.assert && window.assert(nextEdge);
  ...
  triangulatePolygon(edges) {
    ...
    while (edges.length > 3) {
      for (let k = 0; k < edges.length; k++) {
        ...
        window.assert && window.assert(edges[k].getSharedVertex(edges[kx]));
      }
      ...
      for (let i = 0; i < edges.length; i++) {
        ...
        const x =
          endMagnitudeSquared * startMagnitudeSquared -
          startEndProduct * startEndProduct;
        ...
        for (let j = 0; j < edges.length; j++) {
          ...
          if (
            ...
            const u =
              (startMagnitudeSquared * pointEndProduct -
                startEndProduct * pointStartProduct) /
              x;
            const v =
              (endMagnitudeSquared * pointStartProduct -
                startEndProduct * pointEndProduct) /
              x;
    ...
    if (edges.length === 3) {
      this.triangles.push(
        new Triangle(
          edges[0].getSharedVertex(edges[1]),
          edges[1].getSharedVertex(edges[2]),
          edges[0].getSharedVertex(edges[2]),
          edges[2],
          edges[0],
          edges[1]
        )
  ...
  finalize() {
    ...
    window.assert && window.assert(this.firstFrontEdge.triangles.length === 1);
    window.assert && window.assert(lastFrontEdge.triangles.length === 1);
    ...
    for (let i = 0; i < frontEdges.length - 1; i++) {
      ...
      if (
        Utils.triangleAreaSigned(
          secondEdge.endVertex.point,
          firstEdge.endVertex.point,
          firstEdge.startVertex.point
        ) > 1e-10
      ) {
        const newEdge = this.fillBorderTriangle(
          firstEdge,
          secondEdge,
          firstEdge.startVertex,
          firstEdge.endVertex,
          secondEdge.endVertex
        );
    ...
    while (currentSplitEdge !== lastFrontEdge) {
      ...
      // Our min-max-bottomPoint triangle (pivot, no edge to add)
        ...
        window.assert &&
          window.assert(
            currentSplitEdge.startVertex === this.artificialMaxVertex
          );

        // Remove the "bottom" edge connecting both artificial points
        artificialEdges.push(
          nextTriangle.getEdgeOppositeFromVertex(currentSplitEdge.endVertex)
        );

        // Pivot
        currentSplitEdge = nextTriangle.getEdgeOppositeFromVertex(
          currentSplitEdge.startVertex
        );
      }
  ...
  legalizeEdge(edge) {
    ...
    if (
      Utils.pointInCircleFromPoints(
        triangle1.aVertex.point,
        triangle1.bVertex.point,
        triangle1.cVertex.point,
        farVertex2.point
      ) ||
      Utils.pointInCircleFromPoints(
        triangle2.aVertex.point,
        triangle2.bVertex.point,
        triangle2.cVertex.point,
        farVertex1.point
      )
    ) {
      ...
      const triangle1Edge1 = triangle2.getEdgeOppositeFromVertex(
        triangle2.getVertexBefore(farVertex2)
      );
      const triangle1Edge2 = triangle1.getEdgeOppositeFromVertex(
        triangle1.getVertexAfter(farVertex1)
      );
      const triangle2Edge1 = triangle1.getEdgeOppositeFromVertex(
        triangle1.getVertexBefore(farVertex1)
      );
      const triangle2Edge2 = triangle2.getEdgeOppositeFromVertex(
        triangle2.getVertexAfter(farVertex2)
      );
  ...
  static vertexComparison(a, b) {
    window.assert && window.assert(a instanceof Vertex);
    window.assert && window.assert(b instanceof Vertex);
...
class Vertex {
  ...
  constructor(point, index) {
    ...
    window.assert && window.assert(point.isFinite());
    window.assert && window.assert(typeof index === 'number');
...
class Edge {
  ...
  constructor(startVertex, endVertex) {
    ...
    window.assert &&
      window.assert(startVertex !== endVertex, 'Should be different vertices');
  ...
  connectAfter(edge) {
    ...
    window.assert && window.assert(this.endVertex === edge.startVertex);
  ...
  addTriangle(triangle) {
    window.assert && window.assert(triangle instanceof Triangle);
    window.assert && window.assert(this.triangles.length <= 1);
  ...
  removeTriangle(triangle) {
    window.assert && window.assert(triangle instanceof Triangle);
    window.assert && window.assert(_.includes(this.triangles, triangle));
  ...
  getSharedTriangle(otherEdge) {
    window.assert && window.assert(otherEdge instanceof Edge);
  ...
  getSharedVertex(otherEdge) {
    ...
    if (
      this.startVertex === otherEdge.startVertex ||
      this.startVertex === otherEdge.endVertex
    ) {
      return this.startVertex;
    } else {
      window.assert &&
        window.assert(
          this.endVertex === otherEdge.startVertex ||
            this.endVertex === otherEdge.endVertex
        );
  ...
  getOtherVertex(vertex) {
    window.assert && window.assert(vertex instanceof Vertex);
    window.assert &&
      window.assert(vertex === this.startVertex || vertex === this.endVertex);
  ...
  getOtherTriangle(triangle) {
    window.assert && window.assert(triangle instanceof Triangle);
    window.assert && window.assert(this.triangles.length === 2);
  ...
  intersectsConstrainedEdge(vertex, bottomVertex) {
    return Utils.lineSegmentIntersection(
      vertex.point.x,
      vertex.point.y,
      bottomVertex.point.x,
      bottomVertex.point.y,
      this.startVertex.point.x,
      this.startVertex.point.y,
      this.endVertex.point.x,
      this.endVertex.point.y
    );
  }
}
...
class Triangle {
  ...
  constructor(aVertex, bVertex, cVertex, aEdge, bEdge, cEdge) {
    // Type checks
    window.assert && window.assert(aVertex instanceof Vertex);
    window.assert && window.assert(bVertex instanceof Vertex);
    window.assert && window.assert(cVertex instanceof Vertex);
    window.assert && window.assert(aEdge instanceof Edge);
    window.assert && window.assert(bEdge instanceof Edge);
    window.assert && window.assert(cEdge instanceof Edge);

    // Ensure each vertex is NOT in the opposite edge
    window.assert &&
      window.assert(
        aVertex !== aEdge.startVertex && aVertex !== aEdge.endVertex,
        'Should be an opposite edge'
      );
    window.assert &&
      window.assert(
        bVertex !== bEdge.startVertex && bVertex !== bEdge.endVertex,
        'Should be an opposite edge'
      );
    window.assert &&
      window.assert(
        cVertex !== cEdge.startVertex && cVertex !== cEdge.endVertex,
        'Should be an opposite edge'
      );
    ...
    window.assert &&
      window.assert(
        aVertex === bEdge.startVertex || aVertex === bEdge.endVertex,
        'aVertex should be in bEdge'
      );
    window.assert &&
      window.assert(
        aVertex === cEdge.startVertex || aVertex === cEdge.endVertex,
        'aVertex should be in cEdge'
      );
    window.assert &&
      window.assert(
        bVertex === aEdge.startVertex || bVertex === aEdge.endVertex,
        'bVertex should be in aEdge'
      );
    window.assert &&
      window.assert(
        bVertex === cEdge.startVertex || bVertex === cEdge.endVertex,
        'bVertex should be in cEdge'
      );
    window.assert &&
      window.assert(
        cVertex === aEdge.startVertex || cVertex === aEdge.endVertex,
        'cVertex should be in aEdge'
      );
    window.assert &&
      window.assert(
        cVertex === bEdge.startVertex || cVertex === bEdge.endVertex,
        'cVertex should be in bEdge'
      );

    window.assert &&
      window.assert(
        Utils.triangleAreaSigned(aVertex.point, bVertex.point, cVertex.point) >
          0,
  ...
  getVertexOppositeFromEdge(edge) {
    ...
    window.assert &&
      window.assert(
        edge === this.aEdge || edge === this.bEdge || edge === this.cEdge,
    ...
    window.assert &&
      window.assert(
        (this.aEdge.isArtificial() &&
          this.bEdge.isArtificial() &&
          !this.cEdge.isArtificial()) ||
          (this.aEdge.isArtificial() &&
            !this.bEdge.isArtificial() &&
            this.cEdge.isArtificial()) ||
          (!this.aEdge.isArtificial() &&
  ...
  isArtificial() {
    return (
      this.aVertex.isArtificial() ||
      this.bVertex.isArtificial() ||
      this.cVertex.isArtificial()
    );
  }
