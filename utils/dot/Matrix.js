...
class Matrix {
  ...
  constructor(m, n, filler, fast) {
    ...
    if (fast) {
      ...
      if (isArray(filler)) {
        window.assert && window.assert(filler.length === size);
  ...
  getMatrix(i0, i1, j0, j1) {
    ...
    for (let i = i0; i <= i1; i++) {
      for (let j = j0; j <= j1; j++) {
        result.entries[result.index(i - i0, j - j0)] =
          this.entries[this.index(i, j)];
      }
    }
    return result;
  }
  ...
  getArrayRowMatrix(r, j0, j1) {
    ...
    for (let i = 0; i < r.length; i++) {
      for (let j = j0; j <= j1; j++) {
        result.entries[result.index(i, j - j0)] =
          this.entries[this.index(r[i], j)];
      }
    }
    return result;
  }
  ...
  transpose(result) {
    ...
    window.assert && window.assert(result.m === this.n);
    window.assert && window.assert(result.n === this.m);
  ...
  times(matrixOrScalar) {
    ...
    if (matrixOrScalar.isMatrix) {
      ...
      for (i = 0; i < this.m; i++) {
        for (j = 0; j < this.n; j++) {
          result.entries[result.index(i, j)] =
            s * this.entries[this.index(i, j)];
        }
      }
      return result;
    }
  ...
  extractVector2(column) {
    window.assert && window.assert(this.m === 2); // rows should match vector dimension
  ...
  extractVector3(column) {
    window.assert && window.assert(this.m === 3); // rows should match vector dimension
    return new Vector3(
      this.get(0, column),
      this.get(1, column),
      this.get(2, column)
  ...
  extractVector4(column) {
    window.assert && window.assert(this.m === 4); // rows should match vector dimension
    return new Vector4(
      this.get(0, column),
      this.get(1, column),
      this.get(2, column),
  ...
  setVectors3(vectors) {
    ...
    window.assert && window.assert(this.m === m);
    window.assert && window.assert(this.n === n);
