...
function parseRangeWithValue(value: string): RangeWithValue {
  ...
  window.assert && window.assert(tokens.length === 3, `bad query-parameter value, range format is min,max,initial: ${value}`);
  window.assert && window.assert(_.every(tokens, (token: number) => isFinite(token)), `range must be 3 numbers: ${value}`);
