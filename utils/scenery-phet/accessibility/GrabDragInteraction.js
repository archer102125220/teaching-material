...
const gestureHelpTextPatternString =
  SceneryPhetStrings.a11y.grabDrag.gestureHelpTextPattern;
const movableString = SceneryPhetStrings.a11y.grabDrag.movable;
const buttonString = SceneryPhetStrings.a11y.grabDrag.button;
const defaultObjectToGrabString =
  SceneryPhetStrings.a11y.grabDrag.defaultObjectToGrab;
...
class GrabDragInteraction extends EnabledComponent {
  ...
  constructor(node, keyboardDragListener, options) {
    options = merge(
      {
        ...
        addAriaDescribedbyPredicate: (numberOfGrabs) =>
          options.supportsGestureDescription && numberOfGrabs < 2,
    ...
    // @private
    this.grabbableAccessibleName =
      options.grabbableAccessibleName || // if a provided option
      (options.supportsGestureDescription
        ? options.objectToGrabString // otherwise if supporting gesture
    ...
    this.grabFocusHighlight =
      node.focusHighlight || new HighlightFromNode(node);
    this.grabInteractiveHighlight =
      node.interactiveHighlight || new HighlightFromNode(node);
    ...
    this.dragInteractiveHighlight = new HighlightPath(
      this.grabInteractiveHighlight.shape,
      {
        visible: false,
        transformSourceNode:
          this.grabInteractiveHighlight.transformSourceNode || node
      }
    );
    ...
    if (node.interactiveHighlightLayerable) {
      this.grabInteractiveHighlight.parent.addChild(
        this.dragInteractiveHighlight
      );
    }
    ...
    this.grabFocusHighlight.highlightChangedEmitter.addListener(
      onFocusHighlightChange
    );

    const onInteractiveHighlightChange = () => {
      this.dragInteractiveHighlight.setShape(
        this.grabInteractiveHighlight.shape
      );
    };
    this.grabInteractiveHighlight.highlightChangedEmitter.addListener(
    ...
    const grabButtonListener = {
      click: (event) => {
        ...
        if (!guardKeyPressFromDraggable) {
          ...
          if (node.focusHighlightLayerable) {
            ...
            if (
              !this.grabFocusHighlight.parent.hasChild(this.dragFocusHighlight)
            ) {
      ...
      focus: () => {
        ...
        if (this.node.isVoicing && this.showGrabCueNode()) {
          this.voicingFocusUtterance.alert.hintResponse =
            SceneryPhetStrings.a11y.grabDrag.spaceToGrabOrReleaseStringProperty;
    ...
    this.listenersForGrabState =
      options.listenersForGrabState.concat(grabButtonListener);
    ...
    const dragDivListener = new KeyboardListener({
      ...
      callback: (event, keysPressed, listener) => {
        if (listener.keysDown && keysPressed === 'enter') {
          ...
          (!listener.keysDown && keysPressed === 'space') ||
          keysPressed === 'escape'
        ) {
    ...
    const boundUpdateVisibilityForCues =
      this.updateVisibilityForCues.bind(this);
    ...
    this.disposeGrabDragInteraction = () => {
      ...
      this.grabFocusHighlight.highlightChangedEmitter.removeListener(
        onFocusHighlightChange
      );
      this.grabInteractiveHighlight.highlightChangedEmitter.removeListener(
        onInteractiveHighlightChange
      );
      ...
      if (node.interactiveHighlightLayerable) {
        ...
        if (
          this.grabInteractiveHighlight.parent.hasChild(
            this.dragInteractiveHighlight
          )
        ) {
          this.grabInteractiveHighlight.parent.removeChild(
            this.dragInteractiveHighlight
          );
        }
      }
      ...
      this.dragCueNode &&
        this.dragFocusHighlight.focusHighlight.removeChild(this.dragCueNode);
    };
  }
  ...
  turnToGrabbable() {
    ...
    this.baseInteractionUpdate(
      this.grabbableOptions,
      this.listenersForDragState,
      this.listenersForGrabState
    );
