...
export default class GroupSortInteractionModel<ItemModel> extends EnabledComponent {
  ...
  public constructor(providedOptions?: GroupSortInteractionModelOptions<ItemModel>) {
    ...
    window.assert && this.selectedGroupItemProperty.lazyLink(selectedGroupItem => {
      ...
      // window.assert && window.assert( !this.isGroupItemKeyboardGrabbedProperty.value, 'should not change selection when sorting' );

      // If we are PhET-iO instrumented, so should the selection.
      if (Tandem.VALIDATION && selectedGroupItem && options.tandem.supplied) {
        window.assert && window.assert(selectedGroupItem instanceof PhetioObject && selectedGroupItem.isPhetioInstrumented(),
