...
import { Node, Voicing } from '@/utils/scenery/imports';
  ...
  public alert(alertable: TAlertable): void {
    if (this.enabled) {
      if (this.alertToVoicing) {
        window.assert && window.assert(alertable instanceof Utterance, 'If alerting to Voicing, the alertable needs to be an Utterance'); // eslint-disable-line no-simple-type-checking-assertions
