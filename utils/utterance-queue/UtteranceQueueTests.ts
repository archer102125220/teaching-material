...
QUnit.test('Welcome to UtteranceQueueTests!', async assert => {
  window.assert.ok(true, 'UtteranceQueue tests take time, run with ?manualInput and click in the window before the first test');
});
...
QUnit.test('prioritize utterances on add to back', async assert => {
  ...
  window.assert.ok(utteranceQueue['queue'].length === 0, 'nothing man');
  utteranceQueue.addToBack(utterance1);

  window.assert.ok(utteranceQueue['queue'].length === 1, 'one add to back');
  utteranceQueue.addToBack(utterance2);
  window.assert.ok(utteranceQueue['queue'].length === 2, 'one add to back');
  utteranceQueue.addToBack(utterance3);
  window.assert.ok(utteranceQueue['queue'].length === 2, 'one add to back');
  window.assert.ok(utteranceQueue['queue'][0].utterance === utterance1, 'one add to back');
  window.assert.ok(utteranceQueue['queue'][1].utterance === utterance3, 'utterance3 removed utterance1 because cancelOther:true');
  utteranceQueue.addToBack(utterance4);
  window.assert.ok(utteranceQueue['queue'].length === 3, 'one add to back');
  window.assert.ok(utteranceQueue['queue'][0].utterance === utterance1, 'one add to back');
  window.assert.ok(utteranceQueue['queue'][1].utterance === utterance3, 'utterance3 removed utterance1 because cancelOther:true');
  window.assert.ok(utteranceQueue['queue'][2].utterance === utterance4, 'utterance4 does not removed utterance3 because cancelOther:true');

  utteranceQueue.addToBack(utterance5);

  window.assert.ok(utteranceQueue['queue'].length === 4, 'one add to back');
  window.assert.ok(utteranceQueue['queue'][0].utterance === utterance1, 'one add to back');
  window.assert.ok(utteranceQueue['queue'][1].utterance === utterance3, 'utterance3 removed utterance1 because cancelOther:true');
  window.assert.ok(utteranceQueue['queue'][2].utterance === utterance4, 'utterance4 does not removed utterance3 because cancelOther:true');
  window.assert.ok(utteranceQueue['queue'][3].utterance === utterance5, 'utterance4 does not removed utterance3 because cancelOther:true');
  ...
  window.assert.ok(utteranceQueue['queue'].length === 2, 'one add to back');
  window.assert.ok(utteranceQueue['queue'][0].utterance === utterance1, 'one add to back');
  window.assert.ok(utteranceQueue['queue'][1].utterance === utterance5, 'utterance5 kicked utterance4 outta the park.');
});
...
if (testVoicingManager.voicesProperty.value > 0) {
  ...
  QUnit.test('utterance.announcerOptions.voice', async assert => {
    ...
    const done = window.assert.async();
    ...
    testVoicingManager.endSpeakingEmitter.addListener(function myListener() {
      ...
      window.assert.ok(x, 'we should have one');
      window.assert.ok(x.speechSynthesisUtterance.voice === voice, 'voice should match the provided utterance\'s');
...
if (queryParameters.manualInput) {
  ...
  QUnit.test('Basic UtteranceQueue test', async assert => {
    ...
    window.assert.ok(alerts.length === 3, 'Three basic Utterances went through the queue');
  });
  ...
  QUnit.test('cancelUtterance tests', async assert => {
    ...
    testVoicingUtteranceQueue.addToBack(firstUtterance);
    window.assert.ok(alerts[0] === firstUtterance, 'firstUtterance was cancelled');
    window.assert.ok(testVoicingUtteranceQueue['queue'].length === 1, 'There is one Utterance in the queue');
  });
  ...
  QUnit.test('PriorityProperty interruption', async assert => {
    ...
    window.assert.ok(testVoicingUtteranceQueue['queue'].length === 3, 'All three utterances in the queue');

    // make the third Utterance high priority, it should remove the other two Utterances
    thirdUtterance.priorityProperty.value = 2;
    window.assert.ok(testVoicingUtteranceQueue['queue'].length === 1, 'Only the one Utterance remains');
    window.assert.ok(testVoicingUtteranceQueue['queue'][0].utterance === thirdUtterance, 'Only the third Utterance remains');
  });
  ...
  QUnit.test('Announced Utterance can also be in queue and interruption during announcement', async assert => {
    ...
    window.assert.ok(getSpeakingUtterance() === secondUtterance, 'Utterance being announced still observes priorityProperty');
    window.assert.ok(testVoicingUtteranceQueue['queue'].length === 0, 'queue empty after interruption and sending secondUtterance to Announcer');
  });
  ...
  QUnit.test('Higher priority removes earlier Utterances from queue', async assert => {
    ...
    await timeout(timeForSecondUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === secondUtterance, 'The secondUtterance interrupted the firstUtterance because it is higher priority.');
    ...
    window.assert.ok(alerts[0] === secondUtterance, 'secondUtterance spoken in full');
    window.assert.ok(getSpeakingUtterance() === thirdUtterance, 'thirdUtterance spoken after secondUtterance finished');
    // ------------------------------------------------------------------------------------------------------------------
  });
  ...
  QUnit.test('Utterance removed because of self priority reduction before another is added to queue', async assert => {
    ...
    await timeout(timeForFirstUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === thirdUtterance, 'thirdUtterance spoken because firstUtterance.priorityProperty was reduced before adding thirdUtterance to the queue');
  });
  ...
  QUnit.test('Utterance removed because of self priority reduction after another is added to queue', async assert => {
    ...
    // enough time to start speaking either the first or third Utterances
    await timeout(timeForFirstUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === thirdUtterance, 'thirdUtterance spoken because firstUtterance.priorityProperty was reduced after adding thirdUtterance to the queue');
  });
  ...
  QUnit.test('Utterance interruption because self priority reduced while being announced', async assert => {
    ...
    await timeout(timeForFirstUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === firstUtterance);
    ...
    window.assert.ok(alerts[0] === firstUtterance, 'firstUtterance was interrupted because its priority was reduced while it was being announced');

    // enough time for thirdUtterance to start speaking
    await timeout(timeForThirdUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === thirdUtterance, 'thirdUtterance being announced after interrupting firstUtterance');
  });
  ...
  QUnit.test('Utterance interruption during annoumcement because another in the queue made higher priority', async assert => {
    ...
    await timeout(timeForFirstUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === firstUtterance, 'firstUtterance being announced');
    ...
    await timeout(timeForFirstUtterance / 4);
    window.assert.ok(alerts[0] === firstUtterance, 'firstUtterance was interrupted because an Utterance in the queue was made higher priority');
  ...
  QUnit.test('Utterance NOT interrupted because self priority still relatively higher', async assert => {
    ...
    window.assert.ok(alerts.length === 0, 'firstUtterance was not interrupted because priority was set to a value higher than next utterance in queue');
    ...
    window.assert.ok(alerts[0] === firstUtterance, 'firstUtterance finished being announced');
    window.assert.ok(getSpeakingUtterance() === thirdUtterance, 'thirdUtterance being announced after waiting for firstUtterance');
  });

  QUnit.test('announceImmediately', async assert => {

    testVoicingUtteranceQueue.announceImmediately(firstUtterance);
    window.assert.ok(testVoicingUtteranceQueue['queue'].length === 0, 'announceImmediately should be synchronous with voicingManager for an empty queue');

    await timeout(timeForFirstUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === firstUtterance, 'first utterance spoken immediately');
  });
  ...
  QUnit.test('announceImmediately reduces duplicate Utterances in queue', async assert => {
    ...
    await timeout(timeForFirstUtterance / 2);
    window.assert.ok(testVoicingUtteranceQueue['queue'].length === 2, 'announcing firstUtterance immediately should remove the duplicate firstUtterance in the queue');
    window.assert.ok(getSpeakingUtterance() === firstUtterance, 'first utterance is being spoken after announceImmediately');
  });
  ...
  QUnit.test('announceImmediately does nothing when Utterance has low priority relative to queued Utterances', async assert => {
    ...
    // in the queue at all
    window.assert.ok(testVoicingUtteranceQueue['queue'].length === 2, 'only first and second utterances in the queue');
    window.assert.ok(!testVoicingUtteranceQueue.hasUtterance(thirdUtterance), 'thirdUtterance not in queue after announceImmediately');

    await timeout(timeForFirstUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === firstUtterance);
    window.assert.ok(alerts[0] !== thirdUtterance, 'thirdUtterance was not spoken with announceImmediately');
  });
  ...
  QUnit.test('anounceImmediatelety does nothing when Utterance has low priority relative to announcing Utterance', async assert => {
    ...
    window.assert.ok(getSpeakingUtterance() !== thirdUtterance, 'announceImmediately should not interrupt a higher priority utterance');
    window.assert.ok(!testVoicingUtteranceQueue.hasUtterance(thirdUtterance), 'lower priority thirdUtterance should be dropped from the queue');
  });
  ...
  QUnit.test('Utterance spoken with announceImmediately should be interrupted if priority is reduced', async assert => {
    ...
    await timeout(timeForThirdUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === thirdUtterance, 'thirdUtterance is announced immediately');
    ...
    window.assert.ok(alerts[0] === thirdUtterance, 'third utterance was interrupted by reducing its priority');

    await timeout(timeForFirstUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === firstUtterance, 'moved on to next utterance in queue');
  });
  ...
  QUnit.test('Utterance spoken by announceImmediately is interrupted by raising priority of queued utterance', async assert => {
    ...
    window.assert.ok(alerts[0] === thirdUtterance, 'third utterance was interrupted by the next Utterance increasing priority');

    await timeout(timeForFirstUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === firstUtterance, 'moved on to higher priority utterance in queue');
  });
  ...
  QUnit.test('announceImmediately interrupts another Utterance if new Utterance is high priority', async assert => {
    ...
    window.assert.ok(alerts[0] === firstUtterance, 'firstUtterance interrupted because it had lower priority');

    await timeout(timeForThirdUtterance / 2);
    window.assert.ok(getSpeakingUtterance() === thirdUtterance, 'thirdUtterance spoken immediately');
  });
  ...
  QUnit.test('announceImmediately will not interrupt Utterance of equal or lesser priority ', async assert => {
    ...
    await timeout(timeForFirstUtterance / 4);
    window.assert.ok(getSpeakingUtterance() === firstUtterance, 'firstUtterance not interrupted, it has equal priority');
    window.assert.ok(testVoicingUtteranceQueue['queue'][0].utterance === secondUtterance, 'thirdUtterance was added to the front of the queue');
    window.assert.ok(testVoicingUtteranceQueue['queue'].length === 1, 'thirdUtterance was not added to queue and will never be announced');

    await timeout(timeForFirstUtterance / 4 + timeForThirdUtterance / 2);
    window.assert.ok(alerts[0] === firstUtterance, 'firstUtterance spoken in full');
  });
}
