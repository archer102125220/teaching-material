...
export default class Sim extends PhetioObject {
  ...
  public displayOptions?: { container: HTMLElement | Element | null };
  ...
  public constructor(simNameProperty: TReadOnlyProperty<string>, allSimScreens: AnyScreen[], providedOptions?: SimOptions, displayOptions?: { container: HTMLElement | Element | null }) {
    ...
    const simDisplayOptions: SimDisplayOptions = {
      ...
      container: displayOptions?.container || undefined
    };
    ...
    if (typeof this.displayOptions?.container?.parentNode !== 'object' || this.displayOptions?.container?.parentNode === null) {
      document.body.appendChild(this.display.domElement);
    }
  ...
  private finishInit(screens: AnyScreen[]): void {
    ...
    // console.log( this.display?.domElement?.parentElement );
    // $( this.display?.domElement?.parentElement || window ).resize( resizeListener );
    // ( this.display?.domElement?.parentElement || window ).addEventListener( 'resize', resizeListener );
    // ( this.display?.domElement?.parentElement || window ).addEventListener( 'orientationchange', resizeListener );
    // // @ts-expect-error
    // ( this.display?.domElement?.parentElement || window ).visualViewport && ( this.display?.domElement?.parentElement || window ).visualViewport.addEventListener( 'resize', resizeListener );
