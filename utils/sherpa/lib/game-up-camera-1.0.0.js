...
const isGameUpTestHarness =
  window.phet.chipper.queryParameters.gameUpTestHarness;
...
if (isGameUp) {
  ...
  const receiver = (event) => {
    if (
      event.origin.indexOf(suffix, event.origin.length - suffix.length) !==
        -1 ||
      isGameUpTestHarness
    ) {
      if (event.data === 'captureImage') {
        const dataURL =
          window.phet.joist.ScreenshotGenerator.generateScreenshot(
