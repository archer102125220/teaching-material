...
const json = fs.readFileSync(
  'lib/fontawesome-free-5.15.2-desktop/metadata/icons.json',
  'utf8'
);
...
import { Shape } from '@/utils/${style === 'brands' ? '../' : ''}kite/imports';
import ${term} from '@/utils/sherpa/fontawesome-5/${term}.js';
...
keys.forEach((key) => {
  ...
  entry.styles.forEach((style) => {
    ...
    const filename =
      style === 'brands'
        ? `${directory}/${style}/${typeName}String.js`
    ...
    const shapeFilename =
      style === 'brands'
        ? `${directory}/${style}/${typeName}Shape.js`
  ...
  const getImportPath = (element) => {
    if (element.style === 'brands') {
      return `'@/utils/sherpa/fontawesome-5/${element.style}/${element.typeName}Shape.js'`;
    } else {
      return `'@/utils/sherpa/fontawesome-5/${element.typeName}${capitalize(element.style)}Shape.js'`;
    }
  };
  const importStatements = list.map(
    (element) =>
      `import ${element.typeName}_${element.style} from ${getImportPath(element)};`
  );
