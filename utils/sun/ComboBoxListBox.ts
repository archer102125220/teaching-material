...
export default class ComboBoxListBox<T> extends Panel {
  ...
  public constructor(
    ...
    window.assert && window.assert(items.length > 0, 'empty list box is not supported');
    ...
    const fireAction = new PhetioAction(event => {
      ...
      window.assert && window.assert(listItemNode instanceof ComboBoxListItemNode, 'expected a ComboBoxListItemNode'); // eslint-disable-line no-simple-type-checking-assertions
    ...
    items.forEach((item, index) => {
      ...
      window.assert && window.assert(a11yNameProperty);
    ...
    this.visibleProperty.lazyLink(visible => {
      ...
      if (visible) {
        ...
        window.assert && window.assert(selectionWhenListBoxOpened, 'no Node for when list box was opened');
    ...
    const keyboardListener = new KeyboardListener({
      ...
      callback: (event, keysPressed) => {
        ...
        window.assert && window.assert(sceneryEvent, 'event is required for this listener');
        ...
        if (keysPressed === 'escape' || keysPressed === 'tab' || keysPressed === 'shift+tab') {
          ...
          window.assert && window.assert(domEvent, 'domEvent is required for this listener');
          ...
          window.assert && window.assert(focusedItemIndex > -1, 'how could we receive keydown without a focused list item?');
  ...
  private getListItemNode(value: T): ComboBoxListItemNode<T> {
    ...
    window.assert && window.assert(listItemNode, `no item found for value: ${value}`);
    window.assert && window.assert(listItemNode instanceof ComboBoxListItemNode, 'invalid listItemNode'); // eslint-disable-line no-simple-type-checking-assertions
    return listItemNode;
  }
  ...
  private getFocusedItemNode(): ComboBoxListItemNode<T> {
    ...
    window.assert && window.assert(listItemNode, 'no item found that has focus');
    window.assert && window.assert(listItemNode instanceof ComboBoxListItemNode, 'invalid listItemNode'); // eslint-disable-line no-simple-type-checking-assertions
    return listItemNode;
  }
