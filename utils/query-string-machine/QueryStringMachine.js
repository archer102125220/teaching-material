...
(function (root, factory) {
   ...
   * @returns {boolean}
   */
  const isParameterString = (string) =>
    string.length === 0 || string.indexOf('?') === 0;
  ...
  return (function () {
    ...
    const QueryStringMachine = {
      ...
      getForString: function (key, schema, string) {
        ...
        // See https://github.com/phetsims/chipper/issues/743.
        const values =
          schema.private && !privatePredicate() ? [] : getValues(key, string);
        ...
        if (schema.type === 'array' && Array.isArray(value)) {
          ...
          for (let i = 0; i < value.length; i++) {
            ...
            if (
              schema.elementSchema.hasOwnProperty('isValidValue') &&
              !schema.elementSchema.isValidValue(element)
            ) {
              elementsValid = false;
              break;
            }
            if (
              schema.elementSchema.hasOwnProperty('validValues') &&
              !isValidValue(element, schema.elementSchema.validValues)
            ) {
              elementsValid = false;
              break;
            }
      ...
      removeKeyValuePair: function (queryString, key) {
        window.assert &&
          window.assert(
            ...
            `url should be string, but it was: ${typeof queryString}`
          );
        window.assert &&
          window.assert(
            typeof key === 'string',
            `url should be string, but it was: ${typeof key}`
          );
        window.assert &&
          window.assert(
            isParameterString(queryString),
            'queryString should be length 0 or begin with ?'
          );
        window.assert &&
          window.assert(
            key.length > 0,
            'url should be a string with length > 0'
          );
    ...
    const validateSchema = function (key, schema) {
      ...
      queryStringMachineAssert(
        ...
        `type field is required for key: ${key}`
      );

      // type is valid
      queryStringMachineAssert(
        TYPES.hasOwnProperty(schema.type),
        `invalid type: ${schema.type} for key: ${key}`
      );
      ...
      if (schema.hasOwnProperty('parse')) {
        queryStringMachineAssert(
          ...
          `parse must be a function for key: ${key}`
        );
      }
      ...
      queryStringMachineAssert(
        !(
          schema.hasOwnProperty('validValues') &&
          schema.hasOwnProperty('isValidValue')
        ),
        schema,
        key,
      ...
      if (schema.hasOwnProperty('validValues')) {
        queryStringMachineAssert(
          Array.isArray(schema.validValues),
          `isValidValue must be an array for key: ${key}`
        );
      }
      ...
      if (schema.hasOwnProperty('isValidValue')) {
        queryStringMachineAssert(
          typeof schema.isValidValue === 'function',
          `isValidValue must be a function for key: ${key}`
        );
      }
      ...
      if (schema.hasOwnProperty('defaultValue')) {
        queryStringMachineAssert(
          TYPES[schema.type].isValidValue(schema.defaultValue),
          `defaultValue incorrect type: ${key}`
        );
      }

      // validValues have the correct type
      if (schema.hasOwnProperty('validValues')) {
        schema.validValues.forEach((value) =>
          queryStringMachineAssert(
            TYPES[schema.type].isValidValue(value),
            `validValue incorrect type for key: ${key}`
          )
        );
      }
      ...
      if (
        schema.hasOwnProperty('defaultValue') &&
        schema.hasOwnProperty('validValues')
      ) {
        queryStringMachineAssert(
      ...
      if (
        schema.hasOwnProperty('public') &&
        schema.public &&
        schema.type !== 'flag'
      ) {
        queryStringMachineAssert(
          schema.hasOwnProperty('defaultValue'),
          `defaultValue is required when public: true for key: ${key}`
        );
      }
      ...
      validateSchemaProperties(
        key,
        schema,
        TYPES[schema.type].required,
        TYPES[schema.type].optional
      );
    ...
    const validateArraySchema = function (key, schema) {
      ...
      if (schema.hasOwnProperty('separator')) {
        queryStringMachineAssert(
          typeof schema.separator === 'string' && schema.separator.length === 1,
          `invalid separator: ${schema.separator}, for key: ${key}`
        );
      }

      queryStringMachineAssert(
        !schema.elementSchema.hasOwnProperty('public'),
     ...
     */
    const validateSchemaProperties = function (
      key,
      schema,
      requiredProperties,
      optionalProperties
      ...
      requiredProperties.forEach((property) => {
        queryStringMachineAssert(
          ...
          `missing required property: ${property} for key: ${key}`
        );
      });
      ...
      schemaProperties.forEach((property) => {
        queryStringMachineAssert(
          property === 'type' || supportedProperties.indexOf(property) !== -1,
          `unsupported property: ${property} for key: ${key}`
        );
      });
    };
    ...
    const parseValues = function (key, schema, values) {
      ...
      queryStringMachineAssert(
        values.length <= 1,
        `query parameter cannot occur multiple times: ${key}`
      );
      ...
      if (schema.type === 'flag') {
        ...
        queryStringMachineAssert(
          values[0] !== undefined || schema.hasOwnProperty('defaultValue'),
    ...
    const parseArray = function (key, schema, value) {
      ...
      if (value === null) {
        ...
        returnValue = value
          .split(schema.separator || DEFAULT_SEPARATOR)
