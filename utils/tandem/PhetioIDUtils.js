  ...
  window.phetio.PhetioIDUtils = {
    ...
    append: function (phetioID, ...componentNames) {
      componentNames.forEach((componentName) => {
        window.assert &&
          window.assert(
            componentName.indexOf(SEPARATOR) === -1,
    ...
    getComponentName: function (phetioID) {
      window.assert && window.assert(phetioID.length > 0);
    ...
    getScreenID: function (phetioID) {
      ...
      for (let i = 0; i < phetioIDParts.length; i++) {
        ...
        const indexOfScreenMarker = componentPart.indexOf(
    ...
    getGroupElementIndex: function (componentName) {
      window.assert &&
        window.assert(
          componentName.indexOf(window.phetio.PhetioIDUtils.GROUP_SEPARATOR) >=
            0,
          'component name for phetioID should have group element syntax'
        );
      return Number(
        componentName.split(window.phetio.PhetioIDUtils.GROUP_SEPARATOR)[1]
      );
    },
