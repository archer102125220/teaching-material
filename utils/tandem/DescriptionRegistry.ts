...
export default class DescriptionRegistry {
  ...
  public static add( tandem: Tandem, phetioObject: PhetioObject ): void {
    window.assert && window.assert( !DescriptionRegistry.map.has( tandem.phetioID ), 'TandemID already exists in the DescriptionRegistry' );
  ...
  public static remove( phetioObject: PhetioObject ): void {
    ...
    if ( DescriptionRegistry.map.has( tandemID ) ) {
      ...
      window.assert && window.assert( false, 'PhetioObject not found in DescriptionRegistry' );
    }
  }
}
