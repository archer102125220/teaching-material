...
// @ts-expect-error
import Validation, { IsValidValueOptions, Validator } from './Validation';
...
const validate = <T>( value: IntentionalAny, validator: Validator<T>, providedOptions?: IsValidValueOptions ): void => {
  ...
  if ( assert ) {
    ...
    if ( result ) {
      ...
      window.assert && window.assert( false, 'validation failed for value:', value, result, 'prunedValidator:', prunedValidator );
    }
  }
};
