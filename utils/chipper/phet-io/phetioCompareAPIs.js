...
export function handlePhetioCompareAPIs() {
  ...
  const isChildKey = (key) =>
    key !== METADATA_KEY_NAME && key !== DATA_KEY_NAME;
  ...
  const getMetadataValues = (phetioElement, api, _, assert) => {
    const ioTypeName = phetioElement[METADATA_KEY_NAME]
      ? phetioElement[METADATA_KEY_NAME].phetioTypeName || 'ObjectIO'
      : 'ObjectIO';
  ...
  const getMetadataDefaults = (typeName, api, _, assert) => {
    ...
    window.assert && window.assert(entry, `entry missing: ${typeName}`);
    if (entry.supertype) {
      return _.merge(
        getMetadataDefaults(entry.supertype, api, _),
        entry.metadataDefaults
      );
    } else {
  ...
  const _phetioCompareAPIs = (
    ...
    const visit = (trail, reference, proposed, isDesigned) => {
      ...
      if (reference.hasOwnProperty(METADATA_KEY_NAME)) {
        ...
        const referenceCompleteMetadata = getMetadataValues(
          reference,
          referenceAPI,
          _,
          assert
        );
        const proposedCompleteMetadata = getMetadataValues(
          proposed,
          proposedAPI,
          _,
          assert
        ...
        const reportDifferences = (
          ...
          const proposedValue = proposedCompleteMetadata
            ? proposedCompleteMetadata[metadataKey]
            : {};
          ...
          if (referenceValue !== proposedValue) {
            ...
            const ignoreBrokenProposed =
              isOldAPIVersion(proposedAPI) &&
              metadataKey === 'phetioArchetypePhetioID' &&
              referenceValue === null &&
              proposedValue === undefined;

            const ignoreBrokenReference =
              isOldAPIVersion(referenceAPI) &&
              metadataKey === 'phetioArchetypePhetioID' &&
              proposedValue === null &&
            ...
            if (!ignore) {
              if (invalidProposedValue === undefined || isDesignedChange) {
                appendProblem(
                  `${phetioID}.${metadataKey} changed from "${referenceValue}" to "${proposedValue}"`,
                  isDesignedChange
                );
          ...
          if (!proposed._data || !proposed._data.initialState) {
            ...
            const matches = _.isEqualWith(
              ...
              (referenceState, proposedState) => {
                ...
                if (
                  referencesInitialState === referenceState &&
                  proposedInitialState === proposedState
                ) {
                  ...
                  if (phetioID === trail[0] + '.general.model.localeProperty') {
                    ...
                    return referenceState.validValues.every((validValue) =>
                      proposedState.validValues.includes(validValue)
                    );
                  }
                  ...
                  if (phetioID === trail[0] + '.general.controller.input') {
                    return _.isEqual(
                      { ...referenceState, pointers: null },
                      { ...proposedState, pointers: null }
                    );
                  }
                  ...
                  if (
                    phetioID ===
                      'greenhouseEffect.layerModelScreen.model.fluxMeter.wireMeterAttachmentPositionProperty' ||
                    phetioID ===
                      'greenhouseEffect.photonsScreen.model.fluxMeter.wireMeterAttachmentPositionProperty'
                  ) {
                    return true;
                  }
                }
                ...
                // When comparing numbers, don't trigger an error based on floating point inaccuracies. https://github.com/phetsims/aqua/issues/200
                  ...
                  typeof referenceState === 'number' &&
                  typeof proposedState === 'number'
                ) {
                  ...
                  if (referenceState > 10000) {
                    return (
                      referenceState.toPrecision(numberPlaces) ===
                      proposedState.toPrecision(numberPlaces)
                    );
                  } else {
                    return (
                      referenceState.toFixed(numberPlaces) ===
                      proposedState.toFixed(numberPlaces)
                    );
                  }
                }
      ...
      for (const componentName in reference) {
        if (
          reference.hasOwnProperty(componentName) &&
          isChildKey(componentName)
        ) {
      ...
      for (const componentName in proposed) {
        if (
          isDesigned &&
          proposed.hasOwnProperty(componentName) &&
          isChildKey(componentName) &&
          !reference.hasOwnProperty(componentName)
        ) {
          appendProblem(
            `New PhET-iO Element not in reference: ${phetioID}.${componentName}`,
            true
          );
        }
      }
        ...
        if (!proposedAPI.phetioTypes.hasOwnProperty(typeName)) {
          ...
          for (const referenceMethod in referenceMethods) {
            if (referenceMethods.hasOwnProperty(referenceMethod)) {
              if (!proposedMethods.hasOwnProperty(referenceMethod)) {
                appendProblem(
                  `Method missing, type=${typeName}, method=${referenceMethod}`
                );
              } else {
                ...
                const proposedParams =
                  proposedMethods[referenceMethod].parameterTypes;

                if (referenceParams.join(',') !== proposedParams.join(',')) {
                  appendProblem(
                    `${typeName}.${referenceMethod} has different parameter types: [${referenceParams.join(', ')}] => [${proposedParams.join(', ')}]`
                  );
                }

                const referenceReturnType =
                  referenceMethods[referenceMethod].returnType;
                const proposedReturnType =
                  proposedMethods[referenceMethod].returnType;
                if (referenceReturnType !== proposedReturnType) {
                  appendProblem(
                    `${typeName}.${referenceMethod} has a different return type ${referenceReturnType} => ${proposedReturnType}`
                  );
                }
              }
            }
          ...
          if (referenceAPI.version && proposedAPI.version) {
            ...
            const referenceDefaults =
              referenceAPI.phetioTypes[typeName].metadataDefaults;
            const proposedDefaults =
              proposedAPI.phetioTypes[typeName].metadataDefaults;

            Object.keys(referenceDefaults).forEach((key) => {
              if (referenceDefaults[key] !== proposedDefaults[key]) {
                appendProblem(
                  `${typeName} metadata value ${key} changed from "${referenceDefaults[key]}" to "${proposedDefaults[key]}". This may or may not be a breaking change, but we are reporting it just in case.`
                );
              }
            });
          }
