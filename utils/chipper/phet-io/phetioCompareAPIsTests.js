...
qunit.test( 'basics', assert => {
  ...
  window.assert.ok( report.breakingProblems.length === 0, 'no breaking problems when compare to self' );
  window.assert.ok( report.designedProblems.length === 0, 'no designed problems when compare to self' );
  ...
  window.assert.ok( report.breakingProblems.length === 1, 'no breaking problems when compare to self' );
  window.assert.ok( report.designedProblems.length === 0, 'no designed problems when compare to self' );
} );
...
qunit.test( 'designed changes', assert => {
  ...
  let report = phetioCompareAPIs( referenceAPI, proposedAPI );
  window.assert.ok( report.breakingProblems.length === 0, 'no breaking problems when compare to design change' );
  window.assert.ok( report.designedProblems.length === 1, 'no designed problems when changing designed problem' );
  ...
  report = phetioCompareAPIs( referenceAPI, proposedAPI );
  window.assert.ok( report.breakingProblems.length === 0, 'no breaking problems when compare to design change' );
  window.assert.ok( report.designedProblems.length === 2, 'no designed problems when changing designed problem on child' );


  // in the reference, but not the proposed
  referenceAPI.phetioElements.designedElement.otherChild = { _metadata: {} };
  report = phetioCompareAPIs( referenceAPI, proposedAPI );
  window.assert.ok( report.breakingProblems.length === 1, 'cannot delete children like otherChild' );
  ...
  window.assert.ok( report.breakingProblems.length === 0, 'no breaking problems if not testing for them' );
  window.assert.ok( report.designedProblems.length === 0, 'no designed problems if not testing for them' );
} );
...
qunit.test( 'breaking element API changes', assert => {
  ...
  let report = phetioCompareAPIs( referenceAPI, proposedAPI );
  window.assert.ok( report.breakingProblems.length === 1, 'missing an element' );
  window.assert.ok( report.designedProblems.length === 1, 'missing element is also a designed problem' );

  referenceAPI.phetioElements.breakingElement._metadata.phetioDesigned = false;

  report = phetioCompareAPIs( referenceAPI, proposedAPI );
  window.assert.ok( report.breakingProblems.length === 1, 'missing an element' );
  window.assert.ok( report.designedProblems.length === 0, 'not a problem if not designed in the reference' );
  ...
  const testMetadataKeyChange = ( metadataKey, valueThatBreaksAPI ) => {
    ...
    window.assert.ok( report.breakingProblems.length === 1, `it is a breaking change for ${metadataKey} to become ${valueThatBreaksAPI}` );
  };
