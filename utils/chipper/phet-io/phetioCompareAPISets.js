...
import phetioCompareAPIs from './phetioCompareAPIs';
import _jsondiffpatch from '../../../sherpa/lib/jsondiffpatch-v0.3.11.umd';
...
module.exports = async (repos, proposedAPIs, options) => {
  ...
  repos.forEach((repo) => {
    ...
    const referenceAPI = JSON.parse(
      fs.readFileSync(
        `../phet-io-sim-specific/repos/${repo}/${repo}-phet-io-api.json`,
        'utf8'
      )
    );
    ...
    const comparisonData = phetioCompareAPIs(
      referenceAPI,
      proposedAPI,
      _,
      assert,
