...
(function () {
  ...
  const packageObject = _.hasIn(window, 'phet.chipper.packageObject')
    ? window.phet.chipper.packageObject
    : {};
  const packagePhet = packageObject.phet || {};

  // Not all runtimes will have this flag, so be graceful
  const allowLocaleSwitching = _.hasIn(
    window,
    'phet.chipper.allowLocaleSwitching'
  )
  ...
  const QUERY_PARAMETERS_SCHEMA = {
    ...
    supportsDynamicLocale: {
      ...
      defaultValue:
        ...
        (!packageSimFeatures.hasOwnProperty('supportsDynamicLocale') ||
          packageSimFeatures.supportsDynamicLocale)
    },
    ...
    strictAxonDependencies: {
      type: 'boolean',
      defaultValue: packageSimFeatures.hasOwnProperty('strictAxonDependencies')
        ? !!packageSimFeatures.strictAxonDependencies
        : true
    },
  ...
  (function () {
    ...
    window.phet.chipper.queryParameters = window.QueryStringMachine.getAll(
    ...
    window.phet.chipper.brand =
      window.phet.chipper.brand ||
      window.phet.chipper.queryParameters.brand ||
      'adapted-from-phet';
    ...
    if (window.QueryStringMachine.containsKey('locale')) {
      window.phet.chipper.locale = window.phet.chipper.queryParameters.locale;
    } else if (!window.phet.chipper.locale) {
      // Fill in a default
      window.phet.chipper.locale = 'en';
    }

    const stringOverrides = JSON.parse(
      window.phet.chipper.queryParameters.strings || '{}'
    );
     ...
     * @returns {string}
     */
    window.phet.chipper.getStringForBuiltSim = (key) => {
      window.assert &&
        window.assert(
          !!window.phet.chipper.isProduction,
          'expected to be running a built sim'
        );
      window.assert &&
        window.assert(
          !!window.phet.chipper.strings,
          'phet.chipper.strings should be filled out by initialization script'
        );
      window.assert &&
        window.assert(
          !!window.phet.chipper.locale,
          'locale is required to look up the correct strings'
        );
      ...
      let stringMap = window.phet.chipper.strings[window.phet.chipper.locale];
      ...
      if (!stringMap) {
        ...
        stringMap =
          window.phet.chipper.strings[window.phet.chipper.locale.slice(0, 2)];

        if (!stringMap) {
          stringMap = window.phet.chipper.strings.en;
        }
      }
      return window.phet.chipper.mapString(stringMap[key]);
    };
  })();
  ...
  // window.phet.chipper.isProduction =
  //   window.$('meta[name=phet-sim-level]').attr('content') === 'production';
  window.phet.chipper.isProduction = process.env.NODE_ENV === 'production';

  // Are we running in an app?
  window.phet.chipper.isApp =
    window.phet.chipper.queryParameters['phet-app'] ||
    window.phet.chipper.queryParameters['phet-android-app'];
  ...
  (function () {
    ...
    const enableAllAssertions =
      !window.phet.chipper.isProduction &&
      window.phet.chipper.queryParameters.eall;

    // enables basic assertions
    const enableBasicAssertions =
      enableAllAssertions ||
      (!window.phet.chipper.isProduction &&
        window.phet.chipper.queryParameters.ea) ||
      window.phet.chipper.isDebugBuild;
            ...
            _.assignIn(
              {
                continuousTest: JSON.parse(
                  window.phet.chipper.queryParameters.continuousTest
                ),
    ...
    if (window.phet.chipper.queryParameters.postMessageOnError) {
      window.addEventListener('error', (a) => {
        let message = '';
        let stack = '';
        if (a && a.message) {
    ...
    if (window.phet.chipper.queryParameters.postMessageOnBeforeUnload) {
      window.addEventListener('beforeunload', (e) => {
        window.parent &&
          window.parent.postMessage(
            JSON.stringify({
