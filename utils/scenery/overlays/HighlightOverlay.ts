...
export default class HighlightOverlay implements TOverlay {
  ...
  private onInteractiveHighlightChange(): void {
    ...
    if (window.assert) {
      ...
      window.assert(interactiveHighlightNode || (lockedPointerFocus && lockedPointerFocus.trail.lastNode() === this.node),
  ...
  public update(): void {
    ...
    if (!this.display.size.equals(this.focusDisplay.size)) {
      console.log('HighlightOverlay.update');
      // this.focusDisplay.setWidthHeight( this.display.width, this.display.height );

      const width = this.display.domElement?.parentElement?.clientWidth || this.display.width;
      const height = this.display.domElement?.parentElement?.clientHeight || this.display.height;
      // console.log( { width: width, height: height } );
      // console.dir( this.display.domElement?.parentElement );
      this.focusDisplay.setWidthHeight(width, height);
    }
