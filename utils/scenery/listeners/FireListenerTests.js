...
import FireListener from '@/utils/scenery/listeners/FireListener';
...
QUnit.test('Basics', (assert) => {
  ListenerTestUtils.simpleRectangleTest((display, rect, node) => {
    ...
    assert.equal(
      fireCount,
      1,
      'Should NOT fire when pressed and then moved away'
    );
    ...
    assert.equal(
      fireCount,
      1,
      'Should NOT fire when the press misses (even if the release is over)'
    );
    ...
    assert.equal(
      fireCount,
      2,
      'Should fire if the mouse is moved away after press (but moved back before release)'
    );
