...
import PressListener from '@/utils/scenery/listeners/PressListener';
...
QUnit.test('Basics', (assert) => {
  ListenerTestUtils.simpleRectangleTest((display, rect, node) => {
    ...
    assert.equal(
      listener.isHoveringProperty.value,
      false,
      '[1] Is not hovering'
    );
    assert.equal(
      listener.isHighlightedProperty.value,
      false,
      '[1] Is not highlighted'
    );
    ...
    assert.ok(
      listener.pressedTrail.lastNode() === rect,
      '[3] Dragging the proper rectangle'
    );
    ...
    assert.equal(
      listener.isOverProperty.value,
      false,
      '[4] Is NOT over anymore'
    );
    assert.equal(
      listener.isHoveringProperty.value,
      false,
...
QUnit.test('Interruption', (assert) => {
  ListenerTestUtils.simpleRectangleTest((display, rect, node) => {
    ...
    assert.equal(
      listener.isPressedProperty.value,
      true,
      'Is pressed before the interruption'
    );
    listener.interrupt();
    assert.equal(
      listener.isPressedProperty.value,
      false,
      'Is NOT pressed after the interruption'
    );
    listener.dispose();
  });
});
