...
class WebGLBlock extends FittedBlock {
  ...
  initialize(display, renderer, transformRootInstance, filterRootInstance) {
    ...
    this.vertexColorPolygonsProcessor =
      this.vertexColorPolygonsProcessor ||
      new VertexColorPolygons(this.projectionMatrixArray);

    // processor for drawing textured triangles (e.g. Image)
    this.texturedTrianglesProcessor =
      this.texturedTrianglesProcessor ||
      new TexturedTrianglesProcessor(this.projectionMatrixArray);
  ...
  update() {
    ...
    if (
      ...
      this.backingScale !==
        this.originalBackingScale * this.firstDrawable.node._webglScale
    ) {
      this.backingScale =
        this.originalBackingScale * this.firstDrawable.node._webglScale;
      this.dirtyFit = true;
    ...
    for (
      let drawable = this.firstDrawable;
      drawable !== null;
      ...
      if (drawable.visible) {
        ...
        if (drawable.webglRenderer === Renderer.webglTexturedTriangles) {
          ...
          drawable.webglRenderer === Renderer.webglVertexColorPolygons
        ) {
  ...
  addSpriteSheetImage(image, width, height) {
    ...
    if (!sprite) {
      ...
      if (!sprite) {
        ...
        throw new Error(
          'Attempt to load image that is too large for sprite sheets'
        );
      }
    }
    return sprite;
...
class VertexColorPolygons extends Processor {
  ...
  processDrawable(drawable) {
    if (drawable.includeVertices) {
      ...
      while (
        vertexData.length + this.vertexArrayIndex >
  ...
  draw() {
    ...
    // otherwise do a more efficient update that only sends part of the array over
      ...
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        0,
        this.vertexArray.subarray(0, this.vertexArrayIndex)
      );
    }
    ...
    gl.vertexAttribPointer(
      this.shaderProgram.attributeLocations.aColor,
      4,
      gl.FLOAT,
      false,
      stride,
