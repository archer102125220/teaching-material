...
class RectangleWebGLDrawable extends RectangleStatefulDrawable(
  WebGLSelfDrawable
) {
  /**
   * @public
  ...
  update() {
    ...
    if (this.dirtyFill) {
      ...
      if (this.includeVertices) {
        const fill =
          this.node.fill instanceof ReadOnlyProperty
    ...
    if (
      ...
      transformMatrix.multiplyVector2(
        this.lowerRight.setXY(x + width, y + height)
      );
...
RectangleWebGLDrawable.prototype.webglRenderer =
  Renderer.webglVertexColorPolygons;
