...
class SpritesWebGLDrawable extends WebGLSelfDrawable {
  ...
  addSpriteImage(spriteImage) {
    this.spriteImageUVMap[spriteImage.id] = this.spriteSheet.addImage(
      spriteImage.image,
      spriteImage.image.width,
      spriteImage.image.height
    ).uvBounds;
  }
  ...
  draw() {
    ...
    for (let i = 0; i < length; i++) {
      ...
      matrix.multiplyVector2(
        lowerRight.setXY(width - offset.x, height - offset.y)
      );
    ...
    // otherwise do a more efficient update that only sends part of the array over
      ...
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        0,
        this.vertexArray.subarray(0, vertexArrayIndex)
      );
    }
