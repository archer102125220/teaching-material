...
class CanvasBlock extends FittedBlock {
  ...
  initialize(display, renderer, transformRootInstance, filterRootInstance) {
    ...
    this.backingScale =
      renderer & Renderer.bitmaskCanvasLowResolution
        ? 1
        : Utils.backingScale(this.context);
      ...
      if (i > filterRootIndex) {
        if (node._filters.length) {
          ...
          for (let j = 0; j < filters.length; j++) {
            ...
            canUseInternalFilter =
              canUseInternalFilter && filters[j].isDOMCompatible();
          }
  ...
  renderDrawable(drawable) {
    ...
    const branchIndex = this.currentDrawable
      ? drawable.instance.getBranchIndexTo(this.currentDrawable.instance)
      : 0;
    if (this.currentDrawable) {
    ...
    drawable.paintCanvas(
      wrapper,
      drawable.instance.node,
      drawable.instance.relativeTransform.matrix
    );
  ...
  addDrawable(drawable) {
    ...
    for (
      let instance = drawable.instance;
      instance && instance !== this.filterRootInstance;
