...
function hasGapBetweenDrawables(a, b) {
  return (
    ...
    Renderer.isDOM(a.renderer) ||
    Renderer.isDOM(b.renderer)
  );
}
...
function isOpenBefore(drawable) {
  return (
    ...
    !hasGapBetweenDrawables(drawable.previousDrawable, drawable)
  );
}
...
function isOpenAfter(drawable) {
  return (
    ...
    !hasGapBetweenDrawables(drawable, drawable.nextDrawable)
  );
}

// If the change interval will contain any new (added) drawables
function intervalHasNewInternalDrawables(
  interval,
  firstStitchDrawable,
  lastStitchDrawable
) {
...
function intervalHasOldInternalDrawables(
  interval,
  ...
  if (interval.drawableBefore) {
    ...
    return (
      interval.drawableAfter.oldPreviousDrawable !== interval.drawableBefore
    ); // OK for before to be null
  } else {
...
function intervalHasOldInternalBlocks(
  interval,
  firstStitchBlock,
  lastStitchBlock
) {
  const beforeBlock = interval.drawableBefore
    ? interval.drawableBefore.parentDrawable
    : null;
  const afterBlock = interval.drawableAfter
    ? interval.drawableAfter.parentDrawable
    : null;
...
function getLastCompatibleExternalDrawable(interval) {
  ...
  if (firstDrawable) {
    ...
    const cutoffDrawable = interval.nextChangeInterval
      ? interval.nextChangeInterval.drawableBefore.nextDrawable
      : null;
    ...
    while (true) {
      ...
      if (
        nextDrawable !== cutoffDrawable &&
        nextDrawable.renderer === renderer
      ) {
...
class GreedyStitcher extends Stitcher {
  ...
  stitch(
    ...
    if (backbone.blocks.length) {
      ...
      for (
        ...
        if (
          intervalHasOldInternalDrawables(
            interval,
            oldFirstStitchDrawable,
            oldLastStitchDrawable
          )
          ...
          for (
            let removedDrawable = firstRemoval;
            ;
            removedDrawable = removedDrawable.oldNextDrawable
          ) {
        ...
        if (
          intervalHasOldInternalBlocks(interval, veryFirstBlock, veryLastBlock)
        ) {
          const firstBlock =
            interval.drawableBefore === null
              ? backbone.blocks[0]
              : interval.drawableBefore.parentDrawable.nextBlock;
          const lastBlock =
            interval.drawableAfter === null
              ? backbone.blocks[backbone.blocks.length - 1]
              : interval.drawableAfter.parentDrawable.previousBlock;

          for (
            let markedBlock = firstBlock;
            ;
            markedBlock = markedBlock.nextBlock
          ) {
    ...
    if (firstStitchDrawable === null) {
      ...
      this.processBlockLinkedList(
        backbone,
        firstStitchDrawable.pendingParentDrawable,
        lastStitchDrawable.pendingParentDrawable
      );
  ...
  processInterval(backbone, interval, firstStitchDrawable, lastStitchDrawable) {
    ...
    if (
      !intervalHasNewInternalDrawables(
        interval,
        firstStitchDrawable,
        lastStitchDrawable
      )
      ...
      if (interval.drawableBefore && interval.drawableAfter) {
        ...
        const isOpen = !hasGapBetweenDrawables(
          interval.drawableBefore,
    ...
    // otherwise normal operation
      ...
      let drawable = interval.drawableBefore
        ? interval.drawableBefore.nextDrawable
        : firstStitchDrawable;
      ...
      while (true) {
        ...
        if (
          ...
          !drawable.parentDrawable.used &&
          drawable.backbone === backbone &&
        ...
        if (isLast || hasGapBetweenDrawables(drawable, nextDrawable)) {
          if (isFirst) {
            ...
            this.processEdgeCases(
              interval,
              isOpenBefore(subBlockFirstDrawable),
              isOpenAfter(drawable)
            );
          }

          // do the necessary work for each sub-block (adding drawables, linking, using matched blocks)
          this.processSubBlock(
            interval,
            subBlockFirstDrawable,
            drawable,
            matchedBlock,
            isFirst,
            isLast
      ...
      window.assert(
        !openBefore ||
          ...
          firstDrawable.previousDrawable.pendingParentDrawable ===
            lastDrawable.nextDrawable.pendingParentDrawable,
  ...
  processEdgeCases(interval, openBefore, openAfter) {
    ...
    if (interval.drawableBefore !== null && interval.drawableAfter !== null) {
      ...
      const nextAfterBlock =
        interval.nextChangeInterval && interval.nextChangeInterval.drawableAfter
      ...
      if (openBefore && openAfter) {
        ...
        if (this.blockWasAdded || beforeBlock === afterBlock) {
          ...
          // change intervals
          newAfterBlock = this.createBlock(
            interval.drawableAfter.renderer,
            interval.drawableAfter
          );
  ...
  changeExternals(interval, newBlock) {
    ...
    this.notePendingMoves(
      newBlock,
      interval.drawableAfter,
      lastExternalDrawable
    );
    ...
    if (
      !interval.nextChangeInterval ||
      interval.nextChangeInterval.drawableBefore !== lastExternalDrawable
    ) {
    ...
    window.sceneryLog &&
      ...
      window.sceneryLog.GreedyStitcher(
        `linking blocks: ${
          ...
          afterBlock
            ? `${afterBlock.toString()} (${afterDrawable.toString()})`
            : 'null'
        }`
      );
    window.sceneryLog &&
