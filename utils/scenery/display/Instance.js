...
class Instance {
  ...
  initialize(display, trail, isDisplayRoot, isSharedCanvasCacheRoot) {
    ...
    this.relativeTransform =
      this.relativeTransform || new RelativeTransform(this);
    ...
    // stateful
    this.childInsertedListener =
      this.childInsertedListener || this.onChildInserted.bind(this);
    this.childRemovedListener =
      this.childRemovedListener || this.onChildRemoved.bind(this);
    this.childrenReorderedListener =
      this.childrenReorderedListener || this.onChildrenReordered.bind(this);
    this.visibilityListener =
      this.visibilityListener || this.onVisibilityChange.bind(this);
    this.markRenderStateDirtyListener =
      this.markRenderStateDirtyListener || this.markRenderStateDirty.bind(this);
  ...
  updateRenderingState() {
    ...
    this.isUnderCanvasCache =
      this.isSharedCanvasCacheRoot ||
      (this.parent
        ? this.parent.isUnderCanvasCache ||
          this.parent.isInstanceCanvasCache ||
          this.parent.isSharedCanvasCacheSelf
        : false);

    // set up our preferred renderer list (generally based on the parent)
    this.preferredRenderers = this.parent
      ? this.parent.preferredRenderers
      : defaultPreferredRenderers;
    // allow the node to modify its preferred renderers (and those of its descendants)
    if (node._renderer) {
      this.preferredRenderers = Renderer.pushOrderBitmask(
    ...
    const hasFilters =
      this.node.effectiveOpacity !== 1 ||
      node._usesOpacity ||
      this.node._filters.length > 0;
    ...
    const backboneRequired =
      this.isDisplayRoot || (!this.isUnderCanvasCache && requiresSplit);

    // Support either "all Canvas" or "all SVG" opacity/clip
    const applyTransparencyWithBlock =
      !backboneRequired &&
      (hasFilters || hasClip) &&
      ((!hasNonSVGFilter &&
        this.node._rendererSummary.isSubtreeRenderedExclusivelySVG(
          this.preferredRenderers
        )) ||
        (!hasNonCanvasFilter &&
          this.node._rendererSummary.isSubtreeRenderedExclusivelyCanvas(
            this.preferredRenderers
          )));
    const useBackbone = applyTransparencyWithBlock
      ? false
      : backboneRequired || hasFilters || hasClip;
    ...
    // TODO: node._canvasCache hint not defined, always undefined https://github.com/phetsims/scenery/issues/1581
      ...
      if (node._singleCache) {
        ...
        if (this.isSharedCanvasCacheRoot) {
          ...
          this.sharedCacheRenderer = this.isWebGLSupported
            ? Renderer.bitmaskWebGL
            : Renderer.bitmaskCanvas;
        } else {
        ...
        this.groupRenderer = this.isWebGLSupported
          ? Renderer.bitmaskWebGL
          : Renderer.bitmaskCanvas;
      }
    }
    ...
    if (this.node.isPainted()) {
      if (this.isUnderCanvasCache) {
        ...
        if (!this.isWebGLSupported) {
          ...
          supportedNodeBitmask =
            supportedNodeBitmask ^ (supportedNodeBitmask & invalidBitmasks);
        }

        // use the preferred rendering order if specified, otherwise use the default
        this.selfRenderer =
          supportedNodeBitmask &
            Renderer.bitmaskOrder(this.preferredRenderers, 0) ||
          supportedNodeBitmask &
            Renderer.bitmaskOrder(this.preferredRenderers, 1) ||
          supportedNodeBitmask &
            Renderer.bitmaskOrder(this.preferredRenderers, 2) ||
          supportedNodeBitmask &
            Renderer.bitmaskOrder(this.preferredRenderers, 3) ||
    ...
    this.cascadingStateChange =
      wasUnderCanvasCache !== this.isUnderCanvasCache ||
     ...
     */
    this.incompatibleStateChange =
      this.isTransformed !== wasTransformed ||
   ...
   */
  getStateString() {
    const result = `S[ ${this.isDisplayRoot ? 'displayRoot ' : ''}${
    ...
    // pruning so that if no changes would affect a subtree it is skipped
      ...
      if (window.assertSlow) {
        ...
        this.auditChangeIntervals(
          ...
          oldLastInnerDrawable,
          this.firstInnerDrawable,
          this.lastInnerDrawable
        );
      }
  ...
  localSyncTree(selfChanged) {
    ...
    for (let i = 0; i < this.children.length; i++) {
      ...
      const includeChildDrawables =
        childInstance.shouldIncludeInParentDrawables();
      ...
      if (includeChildDrawables) {
        ...
        if (childInstance.firstDrawable) {
          if (currentDrawable) {
            ...
            Drawable.connectDrawables(
              currentDrawable,
              childInstance.firstDrawable,
              this.display
            );
          } else {
      ...
      let isBeforeOpen =
        currentChangeInterval && currentChangeInterval.drawableAfter === null;
      const isAfterOpen =
        firstChildChangeInterval &&
        firstChildChangeInterval.drawableBefore === null;
      const needsBridge =
        childInstance.stitchChangeBefore === frameId &&
        !isBeforeOpen &&
        !isAfterOpen;
        ...
        if (isBeforeOpen) {
          ...
          if (firstChildChangeInterval) {
            if (firstChildChangeInterval.drawableBefore === null) {
              ...
              currentChangeInterval.drawableAfter =
                firstChildChangeInterval.drawableAfter;
              currentChangeInterval.nextChangeInterval =
                firstChildChangeInterval.nextChangeInterval;

              currentChangeInterval =
                childInstance.lastChangeInterval === firstChildChangeInterval
      ...
      if (i === this.children.length - 1) {
        if (
          childInstance.stitchChangeAfter === frameId &&
          !(
            currentChangeInterval &&
            currentChangeInterval.drawableAfter === null
          )
        ) {
          const endingBridge = ChangeInterval.newForDisplay(
            lastUnchangedDrawable,
            null,
            this.display
          );
    ...
    if (window.assertSlow) {
      ...
      for (let j = 0; j < this.children.length; j++) {
        if (
          this.children[j].shouldIncludeInParentDrawables() &&
          this.children[j].firstDrawable
        ) {
      ...
      for (let k = this.children.length - 1; k >= 0; k--) {
        if (
          this.children[k].shouldIncludeInParentDrawables() &&
          this.children[k].lastDrawable
        ) {
  ...
  updateSelfDrawable() {
    if (this.node.isPainted()) {
      ...
      if (
        !this.selfDrawable ||
        (this.selfDrawable.renderer &
          selfRenderer &
          Renderer.bitmaskRendererArea) ===
          0
      ) {
        if (this.selfDrawable) {
          window.sceneryLog &&
        ...
        this.selfDrawable = Renderer.createSelfDrawable(
          this,
          this.node,
          selfRenderer,
  ...
  updateIncompatibleChildInstance(childInstance, index) {
    ...
    const replacementInstance = Instance.createFromPool(
      this.display,
      this.trail.copy().addDescendant(childInstance.node, index),
      false,
      false
    );
  ...
  groupSyncTree(wasStateless) {
    ...
    const groupChanged =
      !!groupRenderer !== !!this.groupDrawable ||
    ...
    if (groupChanged) {
      ...
      this.firstChangeInterval = this.lastChangeInterval =
        ChangeInterval.newForDisplay(null, null, this.display);
    }
    ...
    if (groupRenderer) {
      ...
      this.firstDrawable &&
        Drawable.disconnectBefore(this.firstDrawable, this.display);
      this.lastDrawable &&
        Drawable.disconnectAfter(this.lastDrawable, this.display);

      if (this.isBackbone) {
        if (groupChanged) {
          this.groupDrawable = BackboneDrawable.createFromPool(
            this.display,
            this,
            this.getTransformRootInstance(),
            groupRenderer,
            this.isDisplayRoot
          );
        ...
        if (this.firstChangeInterval) {
          this.groupDrawable.stitch(
            this.firstDrawable,
            this.lastDrawable,
            this.firstChangeInterval,
            this.lastChangeInterval
          );
        }
      } else if (this.isInstanceCanvasCache) {
        if (groupChanged) {
          this.groupDrawable = InlineCanvasCacheDrawable.createFromPool(
            groupRenderer,
            this
          );
        }
  ...
  sharedSyncTree() {
    ...
    if (
      !this.sharedCacheDrawable ||
      this.sharedCacheDrawable.renderer !== sharedCacheRenderer
    ) {
      ...
      this.sharedCacheDrawable = new SharedCanvasCacheDrawable(
        this.trail,
        sharedCacheRenderer,
        this,
        this.sharedCacheInstance
      );
  ...
  prepareChildInstances(wasStateless) {
    ...
    if (wasStateless) {
      ...
      for (let k = 0; k < this.node.children.length; k++) {
        ...
        this.appendInstance(
          Instance.createFromPool(
            this.display,
            this.trail.copy().addDescendant(child, k),
            false,
            false
          )
        );
  ...
  ensureSharedCacheInitialized() {
    ...
    if (!this.sharedCacheInstance) {
      ...
      this.sharedCacheInstance =
        this.display._sharedCanvasInstances[instanceKey];

      // TODO: increment reference counting? https://github.com/phetsims/scenery/issues/1581
      if (!this.sharedCacheInstance) {
        this.sharedCacheInstance = Instance.createFromPool(
          this.display,
          new Trail(this.node),
          false,
          true
  ...
  reorderInstances(minChangeIndex, maxChangeIndex) {
    ...
    this.stitchChangeOnChildren = frameId;
    this.beforeStableIndex = Math.min(
      this.beforeStableIndex,
      minChangeIndex - 1
    );
   ...
   */
  updateVisibility(
    parentGloballyVisible,
    parentGloballyVoicingVisible,
    parentRelativelyVisible,
    updateFullSubtree
  ) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (
        updateFullSubtree ||
        child.visibilityDirty ||
        child.childVisibilityDirty
      ) {
        // if we are a visibility root (isVisibilityApplied===true), disregard ancestor visibility
        child.updateVisibility(
          this.visible,
          this.voicingVisible,
          this.isVisibilityApplied ? true : this.relativeVisible,
          updateFullSubtree
        );
      }
    }
  ...
  attachNodeListeners() {
    ...
    if (!this.isSharedCanvasCachePlaceholder) {
      ...
      this.node.childrenReorderedEmitter.addListener(
        this.childrenReorderedListener
      );
      ...
      this.node.instanceRefreshEmitter.addListener(
        this.markRenderStateDirtyListener
      );
    }
  }
  ...
  detachNodeListeners() {
    ...
    if (!this.isSharedCanvasCachePlaceholder) {
      ...
      this.node.childrenReorderedEmitter.removeListener(
        this.childrenReorderedListener
      );
      ...
      this.node.instanceRefreshEmitter.removeListener(
        this.markRenderStateDirtyListener
      );
    }
  }
  ...
  dispose() {
    ...
    this.sharedCacheDrawable &&
      this.sharedCacheDrawable.disposeImmediately(this.display);
    ...
    if (window.assertSlow) {
      ...
      window.assertSlow(
        this.voicingVisible ===
          _.reduce(
            this.trail.nodes,
            (value, node) => value && node.voicingVisibleProperty.value,
            true
          ),
