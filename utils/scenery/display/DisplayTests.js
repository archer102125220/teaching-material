...
import Circle from '@/utils/kite/nodes/Circle';
import DOM from '@/utils/kite/nodes/DOM';
import Image from '@/utils/kite/nodes/Image';
import Line from '@/utils/kite/nodes/Line';
import Node from '@/utils/kite/nodes/Node';
import Path from '@/utils/kite/nodes/Path';
import Rectangle from '@/utils/kite/nodes/Rectangle';
import Text from '@/utils/kite/nodes/Text';
import CanvasContextWrapper from '@/utils/kite/util/CanvasContextWrapper';
import Display from '@/utils/scenery/display/Display';
import CircleCanvasDrawable from '@/utils/scenery/display/drawables/CircleCanvasDrawable';
import CircleDOMDrawable from '@/utils/scenery/display/drawables/CircleDOMDrawable';
import CircleSVGDrawable from '@/utils/scenery/display/drawables/CircleSVGDrawable';
import DOMDrawable from '@/utils/scenery/display/drawables/DOMDrawable';
import ImageCanvasDrawable from '@/utils/scenery/display/drawables/ImageCanvasDrawable';
import ImageDOMDrawable from '@/utils/scenery/display/drawables/ImageDOMDrawable';
import ImageSVGDrawable from '@/utils/scenery/display/drawables/ImageSVGDrawable';
import LineCanvasDrawable from '@/utils/scenery/display/drawables/LineCanvasDrawable';
import LineSVGDrawable from '@/utils/scenery/display/drawables/LineSVGDrawable';
import PathCanvasDrawable from '@/utils/scenery/display/drawables/PathCanvasDrawable';
import PathSVGDrawable from '@/utils/scenery/display/drawables/PathSVGDrawable';
import RectangleCanvasDrawable from '@/utils/scenery/display/drawables/RectangleCanvasDrawable';
import RectangleDOMDrawable from '@/utils/scenery/display/drawables/RectangleDOMDrawable';
import RectangleSVGDrawable from '@/utils/scenery/display/drawables/RectangleSVGDrawable';
import TextCanvasDrawable from '@/utils/scenery/display/drawables/TextCanvasDrawable';
import TextDOMDrawable from '@/utils/scenery/display/drawables/TextDOMDrawable';
import TextSVGDrawable from '@/utils/scenery/display/drawables/TextSVGDrawable';
import Instance from '@/utils/scenery/display/Instance';
import Renderer from '@/utils/scenery/display/Renderer';
...
QUnit.test('Drawables (Rectangle)', (assert) => {
  ...
  assert.ok(
    r1._drawables.length === 3,
    'After init, should have drawable refs'
  );
  ...
  assert.ok(
    r1._drawables.length === 0,
    'After dispose, should not have drawable refs'
  );

  assert.ok(
    RectangleDOMDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    RectangleSVGDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    RectangleCanvasDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
});
...
QUnit.test('Drawables (Circle)', (assert) => {
  ...
  assert.ok(
    CircleDOMDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    CircleSVGDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    CircleCanvasDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
});
...
QUnit.test('Drawables (Line)', (assert) => {
  ...
  assert.ok(
    LineSVGDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    LineCanvasDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
});
...
QUnit.test('Drawables (Path)', (assert) => {
  ...
  assert.ok(
    PathSVGDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    PathCanvasDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
});
...
QUnit.test('Drawables (Text)', (assert) => {
  ...
  assert.ok(
    TextDOMDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    TextSVGDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    TextCanvasDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
});
...
QUnit.test('Drawables (Image)', (assert) => {
  ...
  assert.ok(
    ImageDOMDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    ImageSVGDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
  assert.ok(
    ImageCanvasDrawable.pool.length > 0,
    'Disposed drawable returned to pool'
  );
});
...
QUnit.test('Renderer order bitmask', (assert) => {
  // init test
  let mask = Renderer.createOrderBitmask(
