...
QUnit.test('focusin/focusout (focus/blur)', (assert) => {
  ...
  window.assert.ok(aGotFocus, 'a should have been focused');
  window.assert.ok(!aLostFocus, 'a should not blur');
  resetFocusVariables();

  b.focus();
  window.assert.ok(bGotFocus, 'b should have been focused');
  window.assert.ok(aLostFocus, 'a should have lost focused');
  resetFocusVariables();

  c.focus();
  window.assert.ok(!bGotFocus, 'b should not receive focus (doesnt bubble)');
  window.assert.ok(cGotFocusIn, 'c should receive a focusin');
  window.assert.ok(bGotFocusIn, 'b should receive a focusin (from bubbling)');
  resetFocusVariables();

  c.blur();
  window.assert.ok(!bGotBlur, 'b should not receive a blur event (doesnt bubble)');
  window.assert.ok(cGotFocusOut, 'c should have received a focusout');
  window.assert.ok(bGotFocusOut, 'c should have received a focusout (from bubbling)');
...
QUnit.test('tab focusin/focusout', (assert) => {
  ...
  buttonA.focus();
  window.assert.ok(buttonA.focused, 'butonA has focus initially');
  ...
  window.assert.ok(buttonC.focused, 'butonC now has focus');
...
QUnit.test('click', (assert) => {
  ...
  window.assert.ok(gotFocus && !gotClick, 'focus first');
  ...
  window.assert.ok(a1.labelContent === null, 'no label on instantiation');
  ...
  window.assert.ok(a1.inputListeners.length === 1, 'accessible listener added');
  ...
  window.assert.ok(a1.labelContent === TEST_LABEL, 'click fired, label set');
...
QUnit.test('click extra', (assert) => {
  ...
  window.assert.ok(a1.inputListeners.length === 0, 'accessible listener removed');
  ...
  window.assert.ok(a1.labelContent === TEST_LABEL_2, 'before click');

  // setting the label redrew the pdom, so get a reference to the new dom element.
  a1.pdomInstances[0].peer.primarySibling.click();
  window.assert.ok(a1.labelContent === TEST_LABEL_2, 'click should not change label');
  ...
  // window.assert.ok( a1.hasInputListener( listener ) === false, 'disposal removed accessible input listeners' );
...
QUnit.test('input', (assert) => {
  ...
  a.pdomInstances[0].peer.primarySibling.focus();
  window.assert.ok(gotFocus && !gotInput, 'focus first');

  dispatchEvent(a.pdomInstances[0].peer.primarySibling, 'input');

  window.assert.ok(gotInput && gotFocus, 'a should have been an input');
...
QUnit.test('change', (assert) => {
  ...
  a.pdomInstances[0].peer.primarySibling.focus();
  window.assert.ok(gotFocus && !gotChange, 'focus first');

  dispatchEvent(a.pdomInstances[0].peer.primarySibling, 'change');

  window.assert.ok(gotChange && gotFocus, 'a should have been an input');
...
QUnit.test('keydown/keyup', (assert) => {
  ...
  a.pdomInstances[0].peer.primarySibling.focus();
  window.assert.ok(gotFocus && !gotKeydown, 'focus first');

  dispatchEvent(a.pdomInstances[0].peer.primarySibling, 'keydown');

  window.assert.ok(gotKeydown && gotFocus, 'a should have had keydown');

  dispatchEvent(a.pdomInstances[0].peer.primarySibling, 'keyup');
  window.assert.ok(gotKeydown && gotKeyup && gotFocus, 'a should have had keyup');
