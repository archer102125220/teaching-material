...
class SimpleDragHandler extends PhetioObject {
  ...
  constructor(options) {
    ...
    this.dragStartAction = new PhetioAction(
      (point, event) => {
        ...
        this.mouseButton =
          event.pointer instanceof Mouse ? event.domEvent.button : undefined;
  ...
  endDrag(event) {
    ...
    this.dragEndAction.execute(
      event ? event.pointer.point : Vector2.ZERO,
      event
    );
  }
  ...
  tryTouchToSnag(event) {
    ...
    if (
      ...
      (this.options.allowTouchSnag === true ||
        this.options.allowTouchSnag(event))
    ) {
