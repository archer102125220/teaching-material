...
const INTERSECTION_ENDPOINT_THRESHOLD_DISTANCE =
  0.1 * VERTEX_COLLAPSE_THRESHOLD_DISTANCE;
const SPLIT_ENDPOINT_THRESHOLD_DISTANCE =
  0.01 * VERTEX_COLLAPSE_THRESHOLD_DISTANCE;
...
class Graph {
  ...
  static deserialize(obj) {
    ...
    graph.edges = obj.edges.map((data) => {
      const edge = new Edge(
        ...
        vertexMap[data.startVertex],
        vertexMap[data.endVertex]
      );
      ...
      const deserializeHalfEdge = (halfEdge, halfEdgeData) => {
        ...
        halfEdge.sortVector = Vector2.Vector2IO.fromStateObject(
          halfEdgeData.sortVector
        );
    ...
    obj.vertices.forEach((data, i) => {
      ...
      vertex.incidentHalfEdges = data.incidentHalfEdges.map(
        (id) => halfEdgeMap[id]
      );
    });
    ...
    obj.boundaries.forEach((data, i) => {
      ...
      boundary.childBoundaries = data.childBoundaries.map(
        (id) => boundaryMap[id]
      );
    });
    ...
    graph.faces = obj.faces.map((data, i) => {
      const face =
        i === 0 ? graph.unboundedFace : new Face(boundaryMap[data.boundary]);
    ...
    obj.edges.forEach((data, i) => {
      ...
      edge.forwardHalf.face =
        data.forwardHalf.face === null ? null : faceMap[data.forwardHalf.face];
      edge.reversedHalf.face =
        data.reversedHalf.face === null
          ? null
          : faceMap[data.reversedHalf.face];
    });
  ...
  addSubpath(shapeId, subpath, options) {
    ...
    const segments = options.ensureClosed
      ? subpath.getFillSegments()
      : subpath.segments;
    let index;
  ...
  replaceEdgeInLoops(edge, forwardHalfEdges) {
    ...
    for (let i = 0; i < forwardHalfEdges.length; i++) {
      reversedHalfEdges.push(
        forwardHalfEdges[forwardHalfEdges.length - 1 - i].getReversed()
      );
    }
    ...
    for (let i = 0; i < this.loops.length; i++) {
      ...
      for (let j = loop.halfEdges.length - 1; j >= 0; j--) {
        ...
        if (halfEdge.edge === edge) {
          const replacementHalfEdges =
            halfEdge === edge.forwardHalf
              ? forwardHalfEdges
              : reversedHalfEdges;
          Array.prototype.splice.apply(
            loop.halfEdges,
            [j, 1].concat(replacementHalfEdges)
          );
        }
      }
    }
          ...
          if (aSegment instanceof Line && bSegment instanceof Line) {
            ...
            if (
              aSegment
                .tangentAt(0)
                .normalized()
                .distance(bSegment.tangentAt(0).normalized()) < 1e-6
            ) {
        ...
        segmentTree.query(edge, (otherEdge) => {
          ...
          if (overlaps !== null && overlaps.length) {
            for (let k = 0; k < overlaps.length; k++) {
              ...
              if (
                Math.abs(overlap.t1 - overlap.t0) > 1e-5 &&
  ...
  splitOverlap(aEdge, bEdge, overlap) {
    ...
    if (bBefore) {
      bBeforeEdge = Edge.pool.create(
        bBefore,
        bEdge.startVertex,
        overlap.a > 0 ? beforeVertex : afterVertex
      );
      newEdges.push(bBeforeEdge);
    }
    if (bAfter) {
      bAfterEdge = Edge.pool.create(
        bAfter,
        overlap.a > 0 ? afterVertex : beforeVertex,
        bEdge.endVertex
      );
    ...
    const aEdges = (aBefore ? [aBeforeEdge] : [])
      .concat([middleEdge])
      .concat(aAfter ? [aAfterEdge] : []);
    const bEdges = (bBefore ? [bBeforeEdge] : [])
      .concat([middleEdge])
      .concat(bAfter ? [bAfterEdge] : []);
    ...
    for (let i = 0; i < bEdges.length; i++) {
      ...
      bForwardHalfEdges.push(
        isForward ? bEdges[i].forwardHalf : bEdges[i].reversedHalf
      );
    }
    ...
    for (let i = this.edges.length - 1; i >= 0; i--) {
      ...
      if (segment instanceof Cubic) {
        ...
        if (selfIntersection) {
          ...
          const startEdge = Edge.pool.create(
            segments[0],
            edge.startVertex,
            vertex
          );
          ...
          intersections = intersections.filter((intersection) => {
            ...
            return (
              Graph.isInternal(
                ...
                intersection.aT,
                aSegment,
                INTERSECTION_ENDPOINT_THRESHOLD_DISTANCE,
                T_THRESHOLD
              ) ||
              Graph.isInternal(
                point,
  ...
  simpleSplit(aEdge, bEdge, aT, bT, point) {
    const aInternal = Graph.isInternal(
      point,
      aT,
      aEdge.segment,
    ...
    const bInternal = Graph.isInternal(
      point,
      bT,
      bEdge.segment,
      SPLIT_ENDPOINT_THRESHOLD_DISTANCE,
    ...
    if (
      aInternal &&
      vertex !== aEdge.startVertex &&
      vertex !== aEdge.endVertex
    ) {
    ...
    if (
      bInternal &&
      vertex !== bEdge.startVertex &&
      vertex !== bEdge.endVertex
    ) {
      ...
      if (entry.start) {
        ...
        segmentTree.query(vertex, (otherVertex) => {
          ...
          if (distance < VERTEX_COLLAPSE_THRESHOLD_DISTANCE) {
            const newVertex = Vertex.pool.create(
              distance === 0
                ? vertex.point
                : vertex.point.average(otherVertex.point)
            );
            ...
            for (let k = this.edges.length - 1; k >= 0; k--) {
              ...
              const startMatches =
                edge.startVertex === vertex || edge.startVertex === otherVertex;
              const endMatches =
                edge.endVertex === vertex || edge.endVertex === otherVertex;

              // Outright remove edges that were between A and B that aren't loops
              if (startMatches && endMatches) {
                if (
                  (edge.segment.bounds.width > 1e-5 ||
                    edge.segment.bounds.height > 1e-5) &&
                  (edge.segment instanceof Cubic ||
                    edge.segment instanceof Arc ||
                    edge.segment instanceof EllipticalArc)
                ) {
                  // Replace it with a new edge that is from the vertex to itself
                  const replacementEdge = Edge.pool.create(
                    edge.segment,
                    newVertex,
                    newVertex
                  );
  ...
  computeBoundaryTree() {
    ...
    for (let i = 0; i < this.outerBoundaries.length; i++) {
      ...
      if (closestEdge === null) {
        ...
        const closestHalfEdge = reversed
          ? closestEdge.reversedHalf
          : closestEdge.forwardHalf;
        const closestBoundary = this.getBoundaryOfHalfEdge(closestHalfEdge);
        closestBoundary.childBoundaries.push(outerBoundary);
      }
    }

    unboundedHoles.forEach(
      this.unboundedFace.recursivelyAddHoles.bind(this.unboundedFace)
    );
    for (let i = 0; i < this.faces.length; i++) {
      const face = this.faces[i];
      if (face.boundary !== null) {
        face.boundary.childBoundaries.forEach(
          face.recursivelyAddHoles.bind(face)
        );
      }
    }
  }
      ...
      for (let j = edges.length - 1; j >= 0; j--) {
        ...
        if (solvedForward && solvedReversed) {
          ...
          for (let k = 0; k < this.shapeIds.length; k++) {
            ...
            windingMap[shapeId] =
              solvedFace.windingMap[shapeId] +
              differential * (solvedForward ? -1 : 1);
          }
    ...
    for (i = 0; i < graph.loops.length; i++) {
      ...
      if (loop.shapeId === SHAPE_ID) {
        ...
        for (j = 0; j < loop.halfEdges.length; j++) {
          ...
          const included = halfEdge.edge.data
            ? options.includeBoundary
            : simplifiedClipAreaShape.containsPoint(
                  halfEdge.edge.segment.positionAt(0.5)
                )
              ? options.includeInterior
