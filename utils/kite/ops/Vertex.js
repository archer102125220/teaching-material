...
class Vertex {
  ...
  sortEdges() {
    ...
    for (let i = 0; i < this.incidentHalfEdges.length; i++) {
      ...
      vectors.push(
        halfEdge.sortVector.setXY(
          halfEdge.getEndTangent().angle,
          halfEdge.getEndCurvature()
        )
      );
    }
  ...
  static edgeComparison(halfEdgeA, halfEdgeB) {
    ...
    // curvature).
    if (
      Math.abs(angleA - angleB) > 1e-5 ||
      (angleA !== angleB &&
        halfEdgeA.edge.segment instanceof Line &&
        halfEdgeB.edge.segment instanceof Line)
    ) {
      ...
      if (Math.abs(curvatureA - curvatureB) > 1e-5) {
        ...
        const t = 1 - 1e-3;
        const curvatureAX = halfEdgeA
          .getDirectionalSegment()
          .subdivided(t)[1]
          .curvatureAt(0);
        const curvatureBX = halfEdgeB
          .getDirectionalSegment()
          .subdivided(t)[1]
          .curvatureAt(0);
